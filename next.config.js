...
const nextConfig = {
  ...
  outputFileTracingIncludes: {
    ...
    '/api/users/custom-fields/generate': ['./package.json', './src/custom-fields/*.js', './src/masters/custom-field-config.js', './src/auth/errors.js',
      './node_modules/moment/**', './node_modules/moment-timezone/**'],
    // new Worker(new URL(...)) targets a real file at runtime, not a static
    // import — make sure the worker script itself is traced into both routes
    // that spawn it (authoring test-run and the live bearer-token dispatcher).
    '/api/dynamic-apis/*/test-run': ['./src/dynamic-apis/sandbox-worker.js'],
    '/api/dynamic/*': ['./src/dynamic-apis/sandbox-worker.js'],
  },
  async headers() {
