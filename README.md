...
Workflow Master at `/workflow_management` includes the source list, Name/Description dialog, cloning, deletion and graph canvas. Managers can add, edit and delete nodes, select templates and roles, change badge styles and node actions, and publish valid drafts with Save Flow. Details-only edits preserve workflow type and code. Managers open the draft when available; readers open the published version first. Graph versions are checked against the active workflow and tenant before loading. Node movement, connection editing and source special-role behavior remain under implementation.
