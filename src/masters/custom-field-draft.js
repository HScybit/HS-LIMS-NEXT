...
export function masterCustomFieldDraft(fields, storedFields = [], previousFields = [], current = {}) {
  ...
  return Object.fromEntries(fields.map(field => {
    if (field.fieldType === 'multi_user_select') return [field.id, Object.hasOwn(current, field.id) ? current[field.id] : customFieldInitialValue(field, storedById.get(field.id))];
