...
function primitive(record, prefix) {
  switch (record[`${prefix}Kind`]) {
    ...
    default: throw new HttpError(409, 'incomplete_product_custom_fields', 'Product Custom Field history is incomplete.');
  }
}
...
export async function loadProductCustomFieldValues(client, identity, productId, revision, expectedCount) {
  ...
  const fields = (await client.query(`SELECT field.field_id AS "fieldId",field.field_revision AS "fieldRevision",field.field_type AS "fieldType",
    ...
    definition.key,definition.label FROM product_version_custom_fields field JOIN custom_field_versions definition
    ...
    WHERE field.organization_id=$1 AND field.product_id=$2 AND field.revision=$3 ORDER BY field.position LIMIT 501`,
  [identity.organization_id, productId, revision])).rows;
  ...
  const items = (await client.query(`SELECT item.field_id AS "fieldId",item.position,item.raw_kind AS "rawKind",item.raw_text AS "rawText",
    ...
    FROM product_version_custom_field_values item
    ...
    WHERE item.organization_id=$1 AND item.product_id=$2 AND item.revision=$3 ORDER BY item.field_id,item.position LIMIT 5001`,
  [identity.organization_id, productId, revision])).rows;
  ...
  for (const item of items) {
    ...
    field.items.push({ value: primitive(item, 'raw'), interpretationState: item.interpretationState,
  ...
  return [...byId.values()].map((field) => {
    ...
    return { fieldId: field.fieldId, fieldRevision: field.fieldRevision, fieldType: field.fieldType, key: field.key, label: field.label,
      ...
      displayValue: primitive(field, 'display'), timeZone: field.timeZone, timeZoneDataVersion: field.timeZoneDataVersion,
...
// Entries have already passed customFieldValuesInput at the Product command boundary.
export async function prepareProductCustomFieldValues(client, identity, { definitions, entries, timeZone, previousFields = [] }) {
  if (entries === undefined) {
    if (definitions.length) throw new HttpError(409, 'product_custom_fields_changed', 'Custom Fields changed. Reload before saving.');
  ...
  if (entries.length !== definitions.length || entries.some((entry) => definitionsById.get(entry.fieldId)?.revision !== entry.fieldRevision)) {
    throw new HttpError(409, 'product_custom_fields_changed', 'Custom Fields changed. Reload before saving.');
  }
    ...
    for (const [itemPosition, value] of values.entries()) {
      ...
      if (value !== '') {
        if (field.fieldType === 'number') {
          ...
          if (!option && !previous) throw new HttpError(400, 'invalid_product_custom_field_option', `${field.label}: Select an available option.`);
        ...
        } else if (field.fieldType === 'lookup') throw new HttpError(400, 'invalid_product_custom_field_lookup', `${field.label}: This lookup has no configured source.`);
      }
      items.push(item);
    }
  }
  if (userIds.size) {
    ...
    if (users.length !== userIds.size) throw new HttpError(400, 'invalid_product_custom_field_user', 'Select users in this organization.');
  }
  if (attachmentIds.size) {
    ...
    if (items.some((item) => item.attachmentId && byId.get(item.attachmentId) !== item.fieldId)) {
      throw new HttpError(400, 'invalid_product_custom_field_attachment', 'Select attachments belonging to these Custom Fields.');
    }
  }
  return { provided: true, count: fields.length, fields, items };
}

export async function appendProductCustomFieldValues(client, identity, productId, revision, capture) {
  ...
  await client.query(`INSERT INTO product_version_custom_fields(organization_id,product_id,revision,${fields.map(([, [column]]) => column).join(',')})
    SELECT $1,$2,$3,field.* FROM unnest(${fields.map(([, [, type]], index) => `$${index + 4}::${type}[]`).join(',')}) AS field`,
  [identity.organization_id, productId, revision, ...fields.map(([key]) => capture.fields.map((field) => field[key]))]);
  ...
  const selected = columns.map(([, [column]]) => column === 'interpretation_state'
    ...
    : ['parsed_date', 'parsed_timestamp'].includes(column)
      ...
      : `item.${column}`);
  await client.query(`INSERT INTO product_version_custom_field_values(organization_id,product_id,revision,${columns.map(([, [column]]) => column).join(',')})
    SELECT $1,$2,$3,${selected.join(',')} FROM unnest(${columns.map(([, [, type]], index) => `$${index + 4}::${type}[]`).join(',')}) AS item(${columns.map(([, [column]]) => column).join(',')})`,
  [identity.organization_id, productId, revision, ...columns.map(([key]) => capture.items.map((item) => item[key]))]);
}
