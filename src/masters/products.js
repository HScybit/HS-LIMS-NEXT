...
export function productInput(input) {
  fieldsOnly(input, ['id', 'requestId', 'revision', 'name', 'key', 'description', 'abbreviation', 'jobTemplateId', 'tagIds', 'sampleCategoryIds', 'customFields', 'customFieldTimeZone']);
  ...
  // Sample Category selection has no source UI yet on this form; omitted (as distinct from explicitly empty) preserves whatever is already set.
  const sampleCategoryIdsProvided = Object.hasOwn(input, 'sampleCategoryIds');
  let sampleCategoryIds;
  if (sampleCategoryIdsProvided) {
    const categories = input.sampleCategoryIds;
    if (!Array.isArray(categories) || categories.length > 500) throw new HttpError(400, 'invalid_product_sample_categories', 'Select at most 500 Sample Categories.');
    sampleCategoryIds = categories.map((id) => uuid(id, 'Sample Category').toLowerCase());
    if (new Set(sampleCategoryIds).size !== sampleCategoryIds.length) throw new HttpError(400, 'invalid_product_sample_categories', 'Each Sample Category can be selected only once.');
  }
  ...
  return { id: uuid(input.id, 'Product').toLowerCase(), requestId: uuid(input.requestId, 'Save request').toLowerCase(),
    ...
    sampleCategoryIdsProvided, sampleCategoryIds, customFieldsProvided, customFields, customFieldTimeZone: zone };
}
...
// sampleCategoryIds has no persisted "was it provided" column (unlike customFieldsProvided), so a loaded
// prior record can't reliably reconstruct original intent; it is deliberately left out of this retry check.
...
// The version-tracking trigger no longer auto-copies this from current state (its FK requires the
// product row first, so it could never see a product's initial categories); the app records them explicitly.
async function appendCategories(client, organizationId, productId, revision, sampleCategoryIds) {
  if (!sampleCategoryIds.length) return;
  await client.query(`INSERT INTO product_version_sample_categories(organization_id,product_id,revision,sample_category_id)
    SELECT $1,$2,$3,sample_category_id FROM unnest($4::uuid[]) AS categories(sample_category_id)`, [organizationId, productId, revision, sampleCategoryIds]);
}
...
export async function saveProduct(client, identity, value, { bulkUpdate = false } = {}) {
  ...
  if (input.sampleCategoryIdsProvided && input.sampleCategoryIds.length) {
    const categories = await client.query('SELECT id FROM sample_categories WHERE organization_id=$1 AND id=ANY($2::uuid[]) AND active ORDER BY id FOR SHARE',
      [identity.organization_id, input.sampleCategoryIds]);
    if (categories.rowCount !== input.sampleCategoryIds.length) throw new HttpError(400, 'invalid_product_sample_categories', 'Select active Sample Categories in this organization.');
  }
  const finalCategoryIds = input.sampleCategoryIdsProvided ? input.sampleCategoryIds
    : (await client.query('SELECT sample_category_id AS id FROM product_sample_categories WHERE organization_id=$1 AND product_id=$2', [identity.organization_id, input.id])).rows.map((row) => row.id);
  ...
  try {
    ...
    await appendCategories(client, identity.organization_id, input.id, input.revision + 1, finalCategoryIds);
    if (input.sampleCategoryIdsProvided) {
      if (input.revision) await client.query('DELETE FROM product_sample_categories WHERE organization_id=$1 AND product_id=$2', [identity.organization_id, input.id]);
      if (input.sampleCategoryIds.length) await client.query('INSERT INTO product_sample_categories(organization_id,product_id,sample_category_id) SELECT $1,$2,sample_category_id FROM unnest($3::uuid[]) AS categories(sample_category_id)',
    ...
    if (error.constraint === 'product_active_sample_category') throw new HttpError(400, 'invalid_product_sample_categories', 'Select active Sample Categories in this organization.');
...
export async function retireProduct(client, identity, input) {
  ...
  await appendCategories(client, identity.organization_id, id, revision + 1, record.sampleCategoryIds);
