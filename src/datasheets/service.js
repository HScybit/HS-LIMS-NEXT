...
export async function loadDatasheet(client, identity, datasheetId, { sampleId, atRevision } = {}) {
  ...
  const capture = await loadCapture(client, identity.organization_id, sheet.templateInstanceId, atRevision);
  const definition = await loadDefinition(client, identity.organization_id, capture.instance.version_id);
