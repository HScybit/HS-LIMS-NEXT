...
import { calculateCapture, filterCaptureValues } from '../templates/calculations.js';
...
export async function loadDatasheet(client, identity, datasheetId, { sampleId, atRevision } = {}) {
  ...
  const modelView = datasheetTemplateView(withImages.model);
  const runtimeView = datasheetCaptureView({ ...capture, values: filterCaptureValues(withImages.model, capture.values, identity.role_ids) });
