...
export async function loadUserFieldLookupSources(fields, previous = new Map(), { signal, request = apiRequest } = {}) {
  ...
  for (let start = 0; start < ids.length; start += 4) {
    ...
    const batch = await Promise.all(ids.slice(start, start + 4).map(async sourceId => {
      ...
      const result = await request(`/api/users/custom-fields/lookup-options?sourceId=${encodeURIComponent(sourceId)}${prior?.revision ? `&revision=${prior.revision}&knownOrganizationId=${encodeURIComponent(prior.organizationId)}` : ''}`, { signal });
      if (result.sourceId !== sourceId || result.unchanged && (!prior || result.organizationId !== prior.organizationId || result.revision !== prior.revision)
