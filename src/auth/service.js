...
export async function publicIdentity(client, identity) {
  const masterModules = (await client.query("SELECT masters_can_read_party('customer') AS customer,masters_can_read_party('vendor') AS vendor,instruments_can_read(NULL) AS instrument,service_agreements_can_read() AS service_agreements")).rows[0];
  return {
