...
export async function publicIdentity(client, identity) {
  const masterModules = (await client.query("SELECT masters_can_read_party('customer') AS customer,masters_can_read_party('vendor') AS vendor,instruments_can_read(NULL) AS instrument,service_agreements_can_read() AS service_agreements,masters_can_read_party('inventory') AS inventory")).rows[0];
  const isPlatformAdministrator = (await client.query('SELECT auth_is_platform_administrator() AS allowed')).rows[0].allowed;
  return {
