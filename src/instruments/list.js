...
const summary = `WITH configuration AS (
  SELECT service.instrument_id,service.position,service.last_performed_on,
    regexp_replace(lower(service.service_code),'[^a-z0-9]','','g') AS key,
  ...
  SELECT *,CASE WHEN key LIKE '%calibration%' THEN 'calibration' ELSE 'maintenance' END AS canonical,
  ...
  FROM configuration WHERE key NOT LIKE '%breakdown%' AND (key LIKE '%calibration%' OR key LIKE '%maintenance%' OR key LIKE '%service%')
), ranked AS (
