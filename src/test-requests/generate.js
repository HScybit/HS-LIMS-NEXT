...
export async function generateTestRequests(client, identity, sampleId, input = {}, { automatic = false } = {}) {
  ...
  const sample = (await client.query('SELECT sample_category_id, sample_type, due_at FROM samples WHERE organization_id = $1 AND id = $2', [identity.organization_id, sampleId])).rows[0];
  ...
  const tests = await client.query(`SELECT test.id, test.test_parameter_id AS "testParameterId", test.method_id AS "methodId", test.decision_rule_id AS "decisionRuleId",
    product.product_id AS "productId", product.sample_category_id AS "sampleCategoryId" FROM sample_tests test JOIN sample_products product
  ...
  const records = tests.rows.map((test, index) => ({ organizationId: identity.organization_id, id: randomUUID(), sampleTestId: test.id,
    ...
    priority, dueAt: dueAt ? new Date(dueAt) : sample.due_at, createdBy: identity.user_id }));
  ...
  return { items: records.map((record) => ({ id: record.id, requestNumber: record.requestNumber, datasheetId: null, workflowRunId: null })), jobs: [] };
}
