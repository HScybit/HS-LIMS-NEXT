...
export async function createSpecifications(client, identity, tests) {
  ...
  const ruleCategoryLinks = rules.size ? await db.select().from(masters.decisionRuleSampleCategories).where(and(eq(masters.decisionRuleSampleCategories.organizationId, organizationId),
    inArray(masters.decisionRuleSampleCategories.decisionRuleId, [...rules.keys()]))) : [];
  ...
  for (const test of tests) {
    ...
    if (rule && (rule.productId !== test.productId || rule.testParameterId !== test.testParameterId
      || (rule.methodId && rule.methodId !== test.methodId) || (ruleCategories.length && !ruleCategories.includes(test.sampleCategoryId)))) missing();
    const snapshot = {
