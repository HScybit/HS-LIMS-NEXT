...
export async function requireMaterialRead(client, identity) {
...
export function materialError(error) {
  if (error.constraint === 'organization_module_access_required') return new HttpError(403, 'inventory_module_access_required', 'Inventory module access is required.');
  const errors = {
...
export async function loadMaterial(client, identity, materialId, { atRevision } = {}) {
  await requireMaterialRead(client, identity); uuid(materialId, 'Material');
  let result;
...
export async function loadMaterialTransaction(client, identity, transactionId) {
  await requireMaterialRead(client, identity); uuid(transactionId, 'Transaction');
...
// Step 10e: an as-of-date snapshot over the existing Materials/Transactions
// pipeline (both already fully built) — the only missing piece was this
// report. The as-of-date boundary is UTC (Q11): PERN's own cutoff is UTC by
// default Postgres/Node configuration despite organizations.timezone
// existing, and Meteor's is undefined-by-code (whatever timezone its server
// process happens to run in) — neither is a deliberate per-organization
// design to follow instead, so this uses the same explicit UTC boundary
// already established elsewhere in this codebase (e.g. retentionDueOn).
export async function getMaterialStockReport(client, identity, { asOnDate } = {}) {
  await requireMaterialRead(client, identity);
  const day = dateOnly(asOnDate ?? new Date().toISOString().slice(0, 10));
  if (day > new Date().toISOString().slice(0, 10)) throw new HttpError(422, 'future_stock_date', 'As on date cannot be in the future.');
  const result = await client.query(`SELECT ${materialColumns},
      material.initial_quantity+coalesce(stock.net_quantity,0) AS "currentQuantity",
      coalesce(stock.inbound_quantity,0) AS "inboundQuantity", coalesce(stock.outbound_quantity,0) AS "outboundQuantity", coalesce(stock.damaged_quantity,0) AS "damagedQuantity"
    FROM ${materialJoins} LEFT JOIN LATERAL (
      SELECT sum(CASE WHEN transaction_type='in' THEN quantity ELSE -quantity END) AS net_quantity,
        sum(quantity) FILTER (WHERE transaction_type='in') AS inbound_quantity,
        sum(quantity) FILTER (WHERE transaction_type='out') AS outbound_quantity,
        sum(quantity) FILTER (WHERE transaction_type='out_damaged') AS damaged_quantity
      FROM material_transactions transaction WHERE transaction.organization_id=material.organization_id AND transaction.material_id=material.id
        AND transaction.created_at < (($2::date + interval '1 day') AT TIME ZONE 'UTC')
    ) stock ON true
    WHERE material.organization_id=$1 AND material.created_at < (($2::date + interval '1 day') AT TIME ZONE 'UTC')
    ORDER BY lower(category.name), lower(material.name), material.id`, [identity.organization_id, day]);
  return { asOnDate: day, items: result.rows };
}
