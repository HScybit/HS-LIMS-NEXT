...
export async function listMaterials(client, identity, input = {}) {
  requireMaterialRead(identity); fieldsOnly(input, ['page', 'pageSize', 'search', 'filters', 'sort']);
...
export async function listMaterialTransactions(client, identity, materialId, input = {}) {
  requireMaterialRead(identity); uuid(materialId, 'Material'); fieldsOnly(input, ['page', 'pageSize', 'type']);
...
export async function materialChoices(client, identity, input = {}) {
  requireMaterialRead(identity); fieldsOnly(input, ['kind', 'page', 'search', 'selectedId', 'materialId']);
...
export async function checkIncomingMaterialBatch(client, identity, materialId, value) {
  requireMaterialRead(identity); uuid(materialId, 'Material'); const batch = materialText(value, 'Batch/Serial No', 150, true);
