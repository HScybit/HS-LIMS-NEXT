...
function pageChromeTemplate(html, stylesheet, xMargin, isNabl) {
...
export function pdfOptions(config, chrome = {}, stylesheet = '', isNabl = false) {
  ...
  return { format: settings.pageSize, landscape: settings.isLandscape, printBackground: true, preferCSSPageSize: false, scale: Number(settings.scale),
    ...
    ...(header?.html || footer?.html ? { displayHeaderFooter: true,
      headerTemplate: pageChromeTemplate(header?.html, stylesheet, settings.xMargin, isNabl), footerTemplate: pageChromeTemplate(footer?.html, stylesheet, settings.xMargin, isNabl) } : {}),
  };
}

export async function renderReportPdf({ html, printConfig, stylesheet, isNabl = false }, { browser, timeoutMs = 30_000 } = {}) {
  ...
  try {
    ...
    if (blockedRequests) throw new HttpError(422, 'report_external_resource', 'The report contains a resource that has not been captured for printing.');
    const bytes = await page.pdf(pdfOptions(printConfig, chrome, stylesheet, isNabl));
