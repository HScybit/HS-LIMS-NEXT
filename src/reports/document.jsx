...
export function renderReportDocument(input, stylesheet) {
  ...
  const bodyClass = ['coa-print-body', data.printConfig.printWithoutSignature ? 'coa-print-body--without-signature' : '',
    data.printConfig.printWithoutImage ? 'coa-print-body--without-image' : ''].filter(Boolean).join(' ');
  // This standalone worker document is rendered outside Next's page runtime.
  // eslint-disable-next-line @next/next/no-head-element
  return '<!doctype html>' + renderToStaticMarkup(<html lang="en"><head><meta charSet="utf-8" /><title>{data.report.reportNumber}</title><style>{stylesheet}</style></head>
    <body><article className="coa-pdf-document coa-printable non_nabl_mode" aria-label={data.report.reportNumber}>
      <div className={bodyClass} data-coa-report-body><TemplateCanvas model={data.model} mode="view" report={data} /></div>
    </article></body></html>);
}
