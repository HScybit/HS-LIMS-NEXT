...
export async function loadReportAssetBatch(client, organizationId, reportIds) {
  ...
  SELECT kind,report_id,slot,id,content_text,image_ids,NULL::text AS media_type,NULL::bytea AS content,(SELECT bytes FROM size) AS image_bytes FROM selected
  ...
  return { assets, metrics: { queryCount: 1, databaseMs, imageValidationMs, rows: rows.length, renderedBytes,
    documents: rows.filter((row) => row.kind === 'document').length, stylesheets: rows.filter((row) => row.kind === 'stylesheet').length, images: sources.size } };
}
