...
import { reportGenerationInput, reportGroups, reissueEditsInput } from './input.js';
...
const reportSummary = (report) => ({ id: report.id, reportNumber: report.reportNumber, revision: report.revision, reportType: report.reportType, groupKey: report.groupKey, status: report.status, isFinalized: report.isFinalized, isNabl: report.isNabl, generatedAt: report.generatedAt });
...
export async function listReports(client, identity, sampleId) {
  ...
  // "Reissue 1", "Reissue 2"... is never stored: it is each batch's rank by
  // when it happened, exactly like Meteor computes it, so relabeling never
  // requires touching already-issued rows.
  const batches = [...new Map(items.filter((item) => item.isReissued).map((item) => [item.reissueBatchId, item.reissuedAt])).entries()].sort(([, left], [, right]) => left - right);
  const labelByBatch = new Map(batches.map(([batchId], index) => [batchId, `Reissue ${index + 1}`]));
  for (const item of items) item.reissueLabel = item.isReissued ? labelByBatch.get(item.reissueBatchId) : null;
  return { items };
}
...
async function generateReportRevisions(client, identity, sampleId, rawInput) {
  ...
  for (const group of groups) {
    ...
    const report = (await client.query(`INSERT INTO sample_reports(organization_id, id, sample_id, template_version_id, report_number, revision, report_type, group_key,
      ...
      customer_reference, received_at, registered_at, due_at, description, is_finalized, is_nabl)
      SELECT $1,$2,sample.id,$4,$5,$6,$7,$8,$9,$10,$11,$12,sample.revision,sample.sample_number,sample.sample_type,sample.category_name,sample.customer_name,sample.customer_address,
        sample.customer_reference,sample.received_at,sample.registered_at,sample.due_at,sample.description,$13,$14
      FROM samples sample WHERE sample.organization_id=$1 AND sample.id=$3
      RETURNING id, report_number AS "reportNumber", revision, report_type AS "reportType", group_key AS "groupKey", status, is_finalized AS "isFinalized", is_nabl AS "isNabl", generated_at AS "generatedAt"`,
    [identity.organization_id, id, sampleId, versions.get(group.templateId), reportNumber, (prior?.revision ?? 0) + 1, input.reportType, group.key,
      group.sampleProductId, group.sampleTestId, identity.user_id, input.requestId, input.finalizeSample, group.isNabl])).rows[0];
...
// Issuing makes a generated, finalized report the official document: it locks the report
// as 'issued' and, scoped to its own group (not the whole sample), supersedes whichever
// report previously held that status for the same group. Reissuing a corrected document
// later is a separate, additional action — this does not itself allow further edits.
export async function issueReport(client, identity, reportId, expectedRevision) {
  requirePermission(identity, 'samples.manage');
  uuid(reportId, 'Report'); revision(expectedRevision);
  const db = database(client);
  ...
  if (report.revision !== expectedRevision) throw new HttpError(409, 'stale_report', 'This report changed. Reload before issuing.');
  if (report.status !== 'draft') throw new HttpError(409, 'report_already_issued', 'This report was already issued.');
  if (!report.isFinalized) throw new HttpError(409, 'report_not_finalized', 'Finalise the sample reports before issuing.');
  const unapproved = await client.query(`SELECT 1 FROM sample_report_tests
    WHERE organization_id=$1 AND report_id=$2 AND (request_status<>'approved' OR datasheet_status<>'approved') LIMIT 1`, [identity.organization_id, reportId]);
  if (unapproved.rowCount) throw new HttpError(409, 'report_tests_unapproved', 'Approve all test requests before issuing this report.');
  const eventId = randomUUID();
  await db.insert(sampleEvents).values({ organizationId: identity.organization_id, id: eventId, sampleId: report.sampleId,
    eventType: 'report_issued', actorUserId: identity.user_id, description: `Issued report ${report.reportNumber} (revision ${report.revision}).` });
  const issued = await client.query(`UPDATE sample_reports SET status='issued', issued_by=$3, issued_at=now()
    WHERE organization_id=$1 AND id=$2 AND status='draft'
    RETURNING id, report_number AS "reportNumber", revision, report_type AS "reportType", group_key AS "groupKey", status, is_finalized AS "isFinalized", generated_at AS "generatedAt", issued_by AS "issuedBy", issued_at AS "issuedAt"`,
  [identity.organization_id, reportId, identity.user_id]);
  if (!issued.rowCount) throw new HttpError(409, 'report_already_issued', 'This report was already issued.');
  await client.query(`UPDATE sample_reports SET status='superseded'
    WHERE organization_id=$1 AND sample_id=$2 AND group_key=$3 AND id<>$4 AND status='issued'`,
  [identity.organization_id, report.sampleId, report.groupKey, reportId]);
  return { item: issued.rows[0] };
}

// Reissue clones every currently-issued report of a sample as one linked
// batch: same tests, same decision limits, same rendered assets, same
// numbers. Only the customer-facing identity fields may be corrected, and
// approval is never reopened. The originals stay issued and visible — a
// reissue supersedes nothing (Meteor's design; PERN has no equivalent).
export async function reissueSampleReports(client, identity, sampleId, rawEdits) {
  requirePermission(identity, 'samples.manage');
  uuid(sampleId, 'Sample');
  const edits = reissueEditsInput(rawEdits);
  const sample = (await client.query('SELECT * FROM samples WHERE organization_id=$1 AND id=$2', [identity.organization_id, sampleId])).rows[0];
  if (!sample) throw new HttpError(404, 'sample_not_found', 'Sample was not found.');
  await requireWorkflowAction(client, identity, { type: 'sample', id: sampleId }, 'reissue');
  const category = (await client.query('SELECT enable_reissue FROM sample_categories WHERE organization_id=$1 AND id=$2', [identity.organization_id, sample.sample_category_id])).rows[0];
  if (!category?.enable_reissue) throw new HttpError(409, 'reissue_not_enabled', 'Report reissue is not enabled for this sample category.');
  const db = database(client);
  const sources = await db.select().from(sampleReports).where(and(scope(sampleReports, identity.organization_id), eq(sampleReports.sampleId, sampleId),
    eq(sampleReports.status, 'issued'), eq(sampleReports.isReissued, false)));
  if (!sources.length) throw new HttpError(409, 'report_not_finalized', 'This sample has no issued reports to reissue.');
  const maxRevisions = new Map((await client.query('SELECT group_key, max(revision) AS revision FROM sample_reports WHERE organization_id=$1 AND sample_id=$2 GROUP BY group_key',
    [identity.organization_id, sampleId])).rows.map((row) => [row.group_key, row.revision]));
  const batchId = randomUUID(); const eventId = randomUUID();
  await db.insert(sampleEvents).values({ organizationId: identity.organization_id, id: eventId, sampleId,
    eventType: 'report_reissued', actorUserId: identity.user_id, description: `Reissued ${sources.length} report${sources.length === 1 ? '' : 's'}.` });
  const reports = [];
  for (const source of sources) {
    const id = randomUUID();
    const nextRevision = (maxRevisions.get(source.groupKey) ?? source.revision) + 1;
    // Copy timestamps inside PostgreSQL: a JavaScript Date would discard the
    // original microseconds and change the historical record.
    const report = (await client.query(`INSERT INTO sample_reports(organization_id, id, sample_id, template_version_id, report_number, revision, report_type, group_key,
      sample_product_id, sample_test_id, generated_by, generated_event_id, sample_revision, sample_number, sample_type, sample_category_name, customer_name, customer_address,
      customer_reference, received_at, registered_at, due_at, description, is_finalized, is_nabl, is_reissued, reissue_source_id, reissue_batch_id, reissued_by, reissued_at, status)
      SELECT $1,$2,source.sample_id,source.template_version_id,source.report_number,$4,source.report_type,source.group_key,
        source.sample_product_id,source.sample_test_id,$5,$6,source.sample_revision,source.sample_number,source.sample_type,source.sample_category_name,
        $7,$8,$9,source.received_at,source.registered_at,source.due_at,source.description,true,source.is_nabl,true,source.id,$10,$5,now(),'draft'
      FROM sample_reports source WHERE source.organization_id=$1 AND source.id=$3
      RETURNING id, report_number AS "reportNumber", revision, report_type AS "reportType", group_key AS "groupKey", status, is_finalized AS "isFinalized", is_nabl AS "isNabl", generated_at AS "generatedAt"`,
    [identity.organization_id, id, source.id, nextRevision, identity.user_id, eventId,
      edits.customerName ?? source.customerName, edits.customerAddress ?? source.customerAddress, edits.customerReference ?? source.customerReference, batchId])).rows[0];
    await client.query(`INSERT INTO sample_report_tests(organization_id, report_id, sample_test_id, sample_product_id, test_request_id, submission_id, specification_id,
        decision_limit_id, display_order, product_code, product_name, request_number, analyst_name, is_accredited, request_status, datasheet_status, completed_at)
      SELECT $1, $2, sample_test_id, sample_product_id, test_request_id, submission_id, specification_id, decision_limit_id, display_order,
        product_code, product_name, request_number, analyst_name, is_accredited, request_status, datasheet_status, completed_at
      FROM sample_report_tests WHERE organization_id=$1 AND report_id=$3`, [identity.organization_id, id, source.id]);
    await client.query(`INSERT INTO sample_report_print_settings(organization_id, report_id, page_size, scale, x_margin, is_landscape, print_header, print_footer,
        print_without_signature, print_without_image, use_custom_top_margin, top_margin, use_custom_bottom_margin, bottom_margin)
      SELECT $1, $2, page_size, scale, x_margin, is_landscape, print_header, print_footer, print_without_signature, print_without_image,
        use_custom_top_margin, top_margin, use_custom_bottom_margin, bottom_margin
      FROM sample_report_print_settings WHERE organization_id=$1 AND report_id=$3`, [identity.organization_id, id, source.id]);
    reports.push(report);
  }
  // Harmless when no report in the batch uses a sample-line widget: the
  // deferred completeness guard only requires a captured line context when
  // one actually applies.
  await client.query('SELECT sample_line_snapshot_reports($1::uuid[])', [reports.map((report) => report.id)]);
  const issued = await client.query(`UPDATE sample_reports SET status='issued', issued_by=$2, issued_at=now()
    WHERE organization_id=$1 AND id=ANY($3::uuid[]) AND status='draft'
    RETURNING id, report_number AS "reportNumber", revision, report_type AS "reportType", group_key AS "groupKey", status, is_finalized AS "isFinalized", is_nabl AS "isNabl",
      generated_at AS "generatedAt", issued_by AS "issuedBy", issued_at AS "issuedAt", is_reissued AS "isReissued", reissue_source_id AS "reissueSourceId", reissue_batch_id AS "reissueBatchId"`,
  [identity.organization_id, identity.user_id, reports.map((report) => report.id)]);
  if (issued.rowCount !== reports.length) throw new HttpError(409, 'report_reissue_failed', 'Reissuing these reports failed. Reload and try again.');
  return { items: issued.rows };
}
