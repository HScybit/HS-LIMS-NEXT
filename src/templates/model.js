...
export function assembleDefinition(records, { forFreeze = false } = {}) {
  const { version, sections, rows, columns, fields, options, expressions, groups, fieldRoleAccess = [] } = records;
  ...
  for (const field of Object.values(model.fieldsById)) {
    ...
    field.options = []; field.editRoleIds = []; field.viewRoleIds = [];
  }
  ...
  for (const grant of fieldRoleAccess) {
    const field = model.fieldsById[grant.fieldId];
    if (!field || !['edit', 'view'].includes(grant.access)) invalid('Role access belongs to a missing field.');
    field[grant.access === 'edit' ? 'editRoleIds' : 'viewRoleIds'].push(grant.roleId);
  }
  ...
  for (const expression of expressions) {
    ...
    for (const reference of expression.references) {
      const target = model.fieldsById[reference.fieldId];
      if (!target || referenceScope(model, field, target) !== reference.scope) invalid('Expression reference has an incompatible repeat scope.');
    }
    const compiled = compileExpression(expression);
    model.expressions[key] = { ...expression, compiled };
    if (expression.purpose === 'calculate') {
      calculations.set(field.id, compiled);
