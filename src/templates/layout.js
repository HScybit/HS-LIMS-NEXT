...
export function cloneLayout(records, model, kind, id, newId) {
  ...
  const result = {
    ...
    expressions: records.expressions.filter((row) => selection.expressions.has(row.id)).map((row) => ({ ...row, id: mapped(row.id), fieldId: mapped(row.fieldId), references: row.references.map((reference) => ({ ...reference, fieldId: mapped(reference.fieldId) })) })),
  };
