...
import { templatePrintConfig } from './print-config.js';
...
export async function loadDefinitions(client, organizationId, versionIds, options = {}) {
  ...
  const { templates, templateVersions: versions, templateSections: sections, templateRows: rows,
    ...
    templateOptions: choices, templateExpressions: expressions, templateExpressionReferences: references, templateRepeatGroups: groups,
    templateFieldRoleAccess: roleAccess } = tables;
  ...
  const versionRows = await query('version', () => db.select({ version: versions, template: templates }).from(versions)
  ...
  const roleAccessRows = await query('field_role_access', () => db.select().from(roleAccess).where(where(roleAccess)).limit(200_001));
  // The bounded lateral lookup keeps fresh-data estimates from scanning every
  // reference in the version for each expression.
  const expressionReferences = db.select().from(references)
    .where(and(eq(references.organizationId, expressions.organizationId), eq(references.versionId, expressions.versionId), eq(references.expressionId, expressions.id)))
    .orderBy(asc(references.alias)).limit(200_001).as('template_expression_references');
  const expressionRows = await query('expressions_dependencies', () => db.select({ expression: expressions, reference: references }).from(expressions)
    .leftJoinLateral(expressionReferences, sql`true`)
    .where(where(expressions)).orderBy(asc(expressions.id), asc(expressionReferences.alias)).limit(200_001));
  ...
  const definitions = new Map(selectedVersions.map((selectedVersion) => [selectedVersion.version.id, {
    records: { version: { ...selectedVersion.version, code: selectedVersion.template.code, active: selectedVersion.template.active,
      printConfig: templatePrintConfig(selectedVersion.template) },
      sections: [], rows: [], columns: [], fields: [], options: [], expressions: [], groups: [], fieldRoleAccess: [] },
  }]));
  ...
  for (const { expression, reference } of expressionRows) {
    const key = `${expression.versionId}:${expression.id}`;
    if (!expressionMap.has(key)) expressionMap.set(key, { ...expression, references: [] });
    if (reference) expressionMap.get(key).references.push({ alias: reference.alias, fieldId: reference.referenceFieldId, scope: reference.referenceScope });
  }
  for (const [key, records] of [['sections', sectionRows], ['rows', layoutRows], ['options', optionRows], ['expressions', expressionMap.values()], ['groups', groupRows], ['fieldRoleAccess', roleAccessRows]]) {
