...
import { calculateCapture, filterCaptureValues, valueKey } from './calculations.js';
...
export async function refreshParameterDetails(client, identity, instanceId, input) {
  ...
  if (prior) {
    ...
    return { instanceId, versionId: instance.version_id, revision: prior.revision, values: filterCaptureValues(model, calculation.values, identity.role_ids), validation: calculation.validation,
  ...
  return { instanceId, versionId: instance.version_id, revision: input.revision + 1, values: filterCaptureValues(model, calculation.values, identity.role_ids), validation: calculation.validation,
...
async function updateCapture(client, identity, instanceId, expectedRevision, inputs) {
  ...
  requireEditableFields(model, entered, identity.role_ids);
  ...
  return { instanceId, versionId, revision: expectedRevision + 1, values: filterCaptureValues(model, calculation.values, identity.role_ids), validation: calculation.validation,
    parameterTitleValuesByRequestId, calculationMs: calculation.durationMs };
}

// Server-enforced edit restriction: a field with a nonempty editRoleIds list can only be written
// by a session holding one of those roles, regardless of what the client's UI allowed.
function requireEditableFields(model, values, sessionRoleIds) {
  for (const value of values) {
    const editRoleIds = model.fieldsById[value.fieldId]?.editRoleIds;
    if (editRoleIds?.length && !editRoleIds.some((roleId) => sessionRoleIds.includes(roleId))) {
      throw new HttpError(403, 'field_edit_forbidden', 'You do not have permission to edit this field.');
    }
  }
}
...
export async function changeRepeat(client, identity, instanceId, expectedRevision, command) {
  ...
  return { instanceId, versionId, revision: nextRevision, occurrences, values: filterCaptureValues(model, calculation.values, identity.role_ids), validation: calculation.validation,
