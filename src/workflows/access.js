...
const capabilities = { view: 'view', edit: 'edit', allocate: 'allocate', addResult: 'execute', printCoa: 'download_report' };
const flags = { edit: 'show_sample_edit', addResult: 'show_add_result' };
const fallbackPermissions = {
  view: ['samples.read', 'samples.manage'], edit: ['samples.manage'], allocate: ['test_requests.allocate', 'samples.manage'],
  addResult: ['datasheets.execute', 'samples.manage'], printCoa: ['samples.manage'], workOnTestRequest: ['datasheets.execute', 'samples.manage'],
};
...
export async function workflowStateAccess(client, identity, owner) {
  ...
  if (!['sample', 'test_request'].includes(owner.type)) throw new HttpError(400, 'invalid_workflow_owner', 'Select a valid workflow owner.');
  const result = await client.query(`SELECT state.*, run.id AS workflow_run_id FROM workflow_runs run JOIN workflow_states state
    ...
    WHERE run.organization_id = $1 AND (($2 = 'sample' AND run.sample_id = $3) OR ($2 = 'test_request' AND run.test_request_id = $3))`,
