...
function requireWorkflowWrite(identity) {
  if (!['samples.manage', 'datasheets.execute', 'approvals.respond', 'documents.manage'].some((permission) => identity.permission_codes?.includes(permission))) {
...
async function requireTransitionReady(client, identity, run, transition, target) {
  ...
  // The two flags below are sample-only concepts, but nothing prevents a
  // document workflow's own states from having them checked in the editor
  // (they're plain, unrestricted checkboxes shared across every owner
  // type). Return early rather than letting a document run fall into the
  // sample-flavored fallback query below, which would resolve zero test
  // requests against a null sample_id and permanently block the transition.
...
async function applyWorkflowTransition(client, identity, run, transition, target, comment) {
  const statement = run.sample_id ? 'SELECT * FROM workflow_apply_sample_transition($1,$2,$3,$4)'
    : run.document_id ? 'SELECT * FROM workflow_apply_document_transition($1,$2,$3,$4)'
    : 'SELECT * FROM workflow_apply_test_request_transition($1,$2,$3,$4)';
