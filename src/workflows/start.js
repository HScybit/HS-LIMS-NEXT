...
export async function startWorkflow(client, identity, owner, sampleCategoryId) {
  const result = await client.query(`SELECT version.id AS version_id, state.id AS state_id, state.generate_test_requests
    FROM sample_category_workflows mapping JOIN workflows workflow
      ON workflow.organization_id = mapping.organization_id AND workflow.id = mapping.workflow_id AND workflow.active
    JOIN workflow_versions version ON version.organization_id = mapping.organization_id AND version.workflow_id = mapping.workflow_id AND version.status = 'published'
    ...
    ORDER BY version.number DESC LIMIT 1`, [identity.organization_id, sampleCategoryId, owner.type]);
  if (!result.rowCount) {
