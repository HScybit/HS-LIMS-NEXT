...
// A document's workflow is a plain per-category link (document_categories.
// workflow_id), not the sample-category "default workflow mapping" system
// startWorkflow() above uses — so this is deliberately a separate, simpler
// function rather than a generalization of it. Returns null (no run
// started) when the category has no workflow configured; the document
// still exists, just without approval tracking. Every new version of a
// document gets its own fresh run (mirroring Meteor: a new version starts
// its own approval cycle rather than inheriting the previous version's
// approved state) — callers pass the new version's id each time.
export async function startDocumentWorkflow(client, identity, documentId, workflowId) {
  if (!workflowId) return null;
  const result = await client.query(`SELECT version.id AS version_id, state.id AS state_id
    ...
    WHERE workflow.organization_id=$1 AND workflow.id=$2 AND workflow.active AND workflow.applies_to='document'
    ORDER BY version.number DESC LIMIT 1`, [identity.organization_id, workflowId]);
  const initial = result.rows[0];
  if (!initial?.state_id) throw new HttpError(422, 'document_workflow_unavailable', 'The configured document workflow is unavailable. Select an active published workflow for this category.');
  const db = database(client);
  const [run] = await db.insert(workflowRuns).values({ organizationId: identity.organization_id, workflowVersionId: initial.version_id,
    documentId, currentStateId: initial.state_id, startedBy: identity.user_id }).returning();
  await db.insert(workflowRunHistory).values({ organizationId: identity.organization_id, workflowRunId: run.id, workflowVersionId: initial.version_id,
    toStateId: initial.state_id, action: 'started', actorUserId: identity.user_id });
  return { id: run.id, versionId: initial.version_id, stateId: initial.state_id };
}
