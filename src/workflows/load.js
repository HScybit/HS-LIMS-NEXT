...
export function requireWorkflowRead(identity) {
  if (!['samples.read', 'samples.manage', 'test_requests.allocate', 'datasheets.execute', 'approvals.respond', 'documents.read', 'documents.manage']
    .some((permission) => identity.permission_codes?.includes(permission))) {
...
export async function workflowConditionSource(client, identity, run) {
  ...
  if (run.document_id) {
    const document = (await client.query(`SELECT document.*, category.name AS document_category_name
      FROM documents document JOIN document_categories category ON category.organization_id=document.organization_id AND category.id=document.document_category_id
      WHERE document.organization_id=$1 AND document.id=$2`, [identity.organization_id, run.document_id])).rows[0];
...
export function canRequestTransition(identity, transition, roles) {
  return ['samples.manage', 'datasheets.execute', 'approvals.respond', 'documents.manage'].some((permission) => identity.permission_codes?.includes(permission))
...
export async function readApprovalCase(client, identity, { caseId = null, runId = null }) {
  ...
  const approval = (await client.query(`SELECT approval.id, approval.status, approval.resolved_at AS "resolvedAt", approval.request_history_id AS "requestHistoryId",
    ...
    run.id AS "workflowRunId", run.sample_id AS "sampleId", run.test_request_id AS "testRequestId", run.document_id AS "documentId",
    coalesce(sample.sample_number, request.request_number, document.name) AS "referenceNumber"
    ...
    LEFT JOIN documents document ON document.organization_id=run.organization_id AND document.id=run.document_id
  ...
  return { ...approval, entityType: approval.sampleId ? 'sample' : approval.documentId ? 'document' : 'test_request', requestedByName: labels.get(approval.requestedById) ?? approval.requestedById,
...
export async function loadWorkflowRun(client, identity, runId) {
  ...
  return { id: run.id, versionId: run.workflow_version_id, sampleId: run.sample_id, testRequestId: run.test_request_id, documentId: run.document_id, revision: run.revision, status: run.status,
