...
export const instrumentVersionServices = pgTable('instrument_version_services', {
  ...scope(), id: uuid('id').notNull(), position: integer('position').notNull(), serviceCode: text('service_code').notNull(), serviceLabel: text('service_label').notNull(),
  ...
  check('instrument_version_service_code_values', sql`${t.serviceCode}=ANY(ARRAY['preventive_maintenance','breakdown','calibration'])`),
]);
