...
export const sampleReports = pgTable('sample_reports', {
  ...
  productContextLineId: uuid('product_context_line_id'), isNabl: boolean('is_nabl').notNull(),
  ...
  isReissued: boolean('is_reissued').notNull().default(false), reissueSourceId: uuid('reissue_source_id'), reissueBatchId: uuid('reissue_batch_id'),
  reissuedBy: uuid('reissued_by'), reissuedAt: time('reissued_at'),
  ...
  foreignKey({ name: 'report_reissue_source_fk', columns: [t.organizationId, t.reissueSourceId], foreignColumns: [t.organizationId, t.id] }),
  foreignKey({ name: 'report_reissued_actor_fk', columns: [t.organizationId, t.reissuedBy], foreignColumns: [memberships.organizationId, memberships.userId] }),
  ...
  index('report_reissue_batch_idx').on(t.organizationId, t.reissueBatchId).where(sql`${t.isReissued}`),
  check('report_revision', sql`${t.revision} > 0 and ${t.sampleRevision} > 0 and length(trim(${t.reportNumber})) between 1 and 100`),
  check('report_reissue', sql`(not ${t.isReissued} and ${t.reissueSourceId} is null and ${t.reissueBatchId} is null and ${t.reissuedBy} is null and ${t.reissuedAt} is null)
    or (${t.isReissued} and ${t.reissueSourceId} is not null and ${t.reissueSourceId} <> ${t.id} and ${t.reissueBatchId} is not null
      and ${t.reissuedBy} is not null and ${t.reissuedAt} is not null and ${t.reissuedAt} >= ${t.generatedAt} and ${t.isFinalized})`),
  check('report_group', sql`(${t.reportType} = 'consolidated' and ${t.groupKey} = 'consolidated:' || (case when ${t.isNabl} then 'nabl' else 'non_nabl' end) and ${t.sampleProductId} is null and ${t.sampleTestId} is null)
    or (${t.reportType} = 'product_wise' and ${t.sampleProductId} is not null and ${t.sampleTestId} is null and ${t.groupKey} = 'product:' || ${t.sampleProductId}::text || ':' || (case when ${t.isNabl} then 'nabl' else 'non_nabl' end))
    or (${t.reportType} = 'parameter_wise' and ${t.sampleProductId} is not null and ${t.sampleTestId} is not null and ${t.groupKey} = 'parameter:' || ${t.sampleTestId}::text || ':' || (case when ${t.isNabl} then 'nabl' else 'non_nabl' end))`),
