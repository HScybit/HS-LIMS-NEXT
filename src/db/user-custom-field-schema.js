...
import { pgTable, uuid, text, integer, boolean, doublePrecision, date, timestamp, index, primaryKey, unique, check, foreignKey, customType } from 'drizzle-orm/pg-core';
...
const scope = () => ({ organizationId: uuid('organization_id').notNull().references(() => organizations.id), subjectUserId: uuid('subject_user_id').notNull(), revision: integer('revision').notNull() });
const versionColumns = table => [table.organizationId, table.subjectUserId, table.revision];

// Revision zero lives only on the membership: absence is not a fabricated capture event.
export const userFieldValueVersions = pgTable('user_field_value_versions', {
  ...scope(), requestId: uuid('request_id').notNull(), previousRevision: integer('previous_revision').notNull(),
  customFieldCount: integer('custom_field_count').notNull(), timeZone: text('time_zone'),
  ...
  primaryKey({ name: 'user_field_value_version_pk', columns: versionColumns(table) }),
  unique('user_field_value_request_key').on(table.organizationId, table.requestId),
  foreignKey({ name: 'user_field_value_subject_fk', columns: [table.organizationId, table.subjectUserId], foreignColumns: [memberships.organizationId, memberships.userId] }),
  foreignKey({ name: 'user_field_value_actor_fk', columns: [table.organizationId, table.savedBy], foreignColumns: [memberships.organizationId, memberships.userId] }),
  check('user_field_value_version', sql`${table.previousRevision}>=0 and ${table.revision}=${table.previousRevision}+1
    and ${table.customFieldCount} between 0 and 500 and (${table.timeZone} is null or (${table.customFieldCount}>0 and length(${table.timeZone}) between 1 and 100))`),
  check('user_field_value_labels', sql`length(trim(${table.username})) between 1 and 100 and length(trim(${table.displayName})) between 1 and 200
    and length(trim(${table.savedByUsername})) between 1 and 100 and length(trim(${table.savedByName})) between 1 and 200`),
]);
...
export const userVersionCustomFields = pgTable('user_version_custom_fields', {
  ...
  primaryKey({ name: 'user_custom_field_pk', columns: capturedFieldColumns(table) }),
  unique('user_custom_field_position').on(...versionColumns(table), table.position),
  foreignKey({ name: 'user_custom_field_version_fk', columns: versionColumns(table), foreignColumns: versionColumns(userFieldValueVersions) }),
  foreignKey({ name: 'user_custom_field_definition_fk', columns: [table.organizationId, table.fieldId, table.fieldRevision],
    foreignColumns: [customFieldVersions.organizationId, customFieldVersions.fieldId, customFieldVersions.revision] }),
  check('user_custom_field_shape', sql`${table.position} between 0 and 499 and ${table.valueCount} between 0 and 500
    and (${table.isArray} or ${table.valueCount}=1)
    and ${table.fieldType} in (${sql.raw(customFieldTypes.map(({ value }) => `'${value}'`).join(','))})`),
  check('user_custom_field_display', sql`num_nonnulls(${table.displayText},${table.displayNumber},${table.displayBoolean})=1
  ...
  check('user_custom_field_zone', sql`(${table.fieldType} in ('date','date_time') and ${table.timeZone} is not null
  ...
  index('user_custom_field_definition').on(table.organizationId, table.fieldId, table.subjectUserId, table.revision),
]);

export const userVersionCustomFieldValues = pgTable('user_version_custom_field_values', {
  ...
  userUsername: text('user_username'), userName: text('user_name'),
  lookupSourceId: uuid('lookup_source_id'), lookupRevision: integer('lookup_revision'), lookupLineId: text('lookup_line_id'),
}, (table) => [
  primaryKey({ name: 'user_custom_value_pk', columns: [...capturedFieldColumns(table), table.position] }),
  foreignKey({ name: 'user_custom_value_field_fk', columns: capturedFieldColumns(table), foreignColumns: capturedFieldColumns(userVersionCustomFields) }),
  foreignKey({ name: 'user_custom_value_option_fk', columns: [table.organizationId, table.fieldId, table.optionRevision, table.optionId],
  ...
  foreignKey({ name: 'user_custom_value_user_fk', columns: [table.organizationId, table.userId], foreignColumns: [memberships.organizationId, memberships.userId] }),
  foreignKey({ name: 'user_custom_value_attachment_fk', columns: [table.organizationId, table.attachmentId], foreignColumns: [customFieldAttachments.organizationId, customFieldAttachments.id] }),
  foreignKey({ name: 'user_custom_value_lookup_fk', columns: [table.organizationId, table.lookupSourceId, table.lookupRevision, table.lookupLineId],
  ...
  check('user_custom_value_lookup_reference', sql`num_nonnulls(${table.lookupSourceId},${table.lookupRevision},${table.lookupLineId})=0
  ...
  check('user_custom_value_user_observation', sql`(${table.userId} is null and num_nonnulls(${table.userUsername},${table.userName})=0)
    or (${table.userId} is not null and ${table.userUsername} is not null and ${table.userName} is not null
      and length(trim(${table.userUsername})) between 1 and 100 and length(trim(${table.userName})) between 1 and 200)`),
  check('user_custom_value_raw', sql`${table.position} between 0 and 499 and num_nonnulls(${table.rawText},${table.rawNumber},${table.rawBoolean})=1
  ...
  check('user_custom_value_number_text', sql`case when ${table.rawKind}='number' then
  ...
  check('user_custom_value_interpretation', sql`${table.interpretationState} in ('empty','valid','invalid','out_of_range')
  ...
  index('user_custom_value_raw_search').on(table.organizationId, table.fieldId, sql`md5(${table.rawText})`, table.subjectUserId, table.revision).where(sql`${table.rawText} is not null`),
]);
