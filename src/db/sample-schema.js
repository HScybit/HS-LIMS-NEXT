...
export const samples = pgTable('samples', {
  ...
  complaintRca: text('complaint_rca'), complaintCapa: text('complaint_capa'),
...
export const workflowRuns = pgTable('workflow_runs', {
  ...identity(), workflowVersionId: uuid('workflow_version_id').notNull(), sampleId: uuid('sample_id'), testRequestId: uuid('test_request_id'), documentId: uuid('document_id'), currentStateId: uuid('current_state_id').notNull(),
...
}, (t) => [key(t), link(t, t.workflowVersionId, workflowVersions), link(t, t.sampleId, samples), link(t, t.testRequestId, testRequests),
  foreignKey({ name: 'workflow_run_document_fk', columns: [t.organizationId, t.documentId], foreignColumns: [documents.organizationId, documents.id] }), actor(t, t.startedBy),
  ...
  unique('workflow_run_document_key').on(t.organizationId, t.documentId),
  unique('workflow_run_version_key').on(t.organizationId, t.id, t.workflowVersionId),
  check('workflow_run_owner', sql`num_nonnulls(${t.sampleId}, ${t.testRequestId}, ${t.documentId}) = 1`),
...
export const sampleEvents = pgTable('sample_events', {
  ...
  check('sample_event_type', sql`${t.eventType} in ('sample_registered', 'test_requests_generated', 'test_request_assigned', 'datasheet_created', 'datasheet_submitted', 'reports_generated', 'reports_finalized', 'report_issued', 'report_reissued', 'datasheet_method_added', 'datasheet_method_voided', 'test_request_job_created', 'sample_updated')`)]);
