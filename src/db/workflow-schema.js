...
export const workflows = pgTable('workflows', {
  ...
  check('workflow_metadata', sql`length(trim(${t.code})) between 1 and 64 and length(trim(${t.name})) between 1 and 200 and ${t.appliesTo} in ('sample', 'test_request', 'instrument_service', 'document')`)]);
...
export const workflowMetadataVersions = pgTable('workflow_metadata_versions', {
  ...
  check('workflow_metadata_history_fields', sql`length(trim(${t.code})) between 1 and 64 and length(trim(${t.name})) between 1 and 200
    and ${t.appliesTo} in ('sample','test_request','instrument_service','document') and (${t.requestedAppliesTo} is null or ${t.requestedAppliesTo} in ('sample','test_request','instrument_service','document'))
