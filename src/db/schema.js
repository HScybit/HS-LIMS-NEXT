...
import {
  ...
  uniqueIndex, index, check, foreignKey, date, numeric,
} from 'drizzle-orm/pg-core';

const time = (name) => timestamp(name, { withTimezone: true, mode: 'date' });
const id = () => uuid('id').primaryKey().defaultRandom();

// `active` stays the functional gate every RLS/session check already relies on; `status` is
// the richer, three-state field the platform-administrator Organizations form/listing manage
// (see drizzle/0220_organization_details.sql), kept in sync with `active` by the two
// platform_create_organization/platform_update_organization SQL functions.
export const organizations = pgTable('organizations', {
  ...
  activeFromDate: date('active_from_date', { mode: 'string' }),
  activeTillDate: date('active_till_date', { mode: 'string' }),
  accountType: text('account_type').notNull().default('saas'),
  ...
  subscriptionCost: numeric('subscription_cost'),
  customDevelopmentCost: numeric('custom_development_cost'),
  createdAt: time('created_at').notNull().defaultNow(),
  updatedAt: time('updated_at').notNull().defaultNow(),
}, (table) => [
  uniqueIndex('organizations_code_key').on(sql`lower(${table.code})`),
  uniqueIndex('organizations_domain_key').on(sql`lower(${table.domain})`).where(sql`${table.domain} is not null`),
  check('organization_status', sql`${table.status} in ('active','suspended','archived')`),
  check('organization_account_type', sql`${table.accountType} in ('saas','enterprise')`),
  check('organization_active_matches_status', sql`${table.active} = (${table.status}='active')`),
  check('organization_active_dates', sql`${table.activeFromDate} is null or ${table.activeTillDate} is null or ${table.activeFromDate}<=${table.activeTillDate}`),
  check('organization_costs', sql`(${table.subscriptionCost} is null or (${table.subscriptionCost}>=0 and ${table.subscriptionCost} not in ('NaN'::numeric,'Infinity'::numeric,'-Infinity'::numeric)))
    and (${table.customDevelopmentCost} is null or (${table.customDevelopmentCost}>=0 and ${table.customDevelopmentCost} not in ('NaN'::numeric,'Infinity'::numeric,'-Infinity'::numeric)))`),
  check('organization_text_fields', sql`(${table.domain} is null or length(${table.domain}) between 1 and 200) and (${table.purchaseOrderNumber} is null or length(${table.purchaseOrderNumber})<=100)
    and (${table.address} is null or length(${table.address})<=4000) and (${table.contactPerson} is null or length(${table.contactPerson})<=200)
    and (${table.contactPhone} is null or length(${table.contactPhone})<=50) and (${table.projectManager} is null or length(${table.projectManager})<=200)
    and (${table.salesPerson} is null or length(${table.salesPerson})<=200) and (${table.pricingPlan} is null or length(${table.pricingPlan})<=200)`),
]);
