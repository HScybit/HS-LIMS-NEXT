...
import { generateProductScheme, generateParameterScheme, generateMethodScheme, generateUserScheme } from './product-generation.js';
...
async function generate() {
  ...
  if (workerData.kind !== undefined && !['product', 'parameter', 'method', 'user'].includes(workerData.kind)) throw new Error('Unsupported Custom Field master.');
  const scheme = workerData.kind === 'user' ? generateUserScheme : workerData.kind === 'method' ? generateMethodScheme : workerData.kind === 'parameter' ? generateParameterScheme : generateProductScheme;
  const generated = [];
