export async function uploadCustomFieldFile(field, file, requestId, signal, { userFields = false } = {}) {
  ...
  const response = await fetch(userFields ? '/api/users/custom-fields/attachments' : '/api/custom-fields/attachments', {
