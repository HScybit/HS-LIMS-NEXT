...
export async function loadLaboratorySettings(client, identity) {
  ...
  const stored = (await client.query(`SELECT auto_create_jobs AS "autoCreateJobs",result_summary_template_id AS "resultSummaryTemplateId",
  ...
  return { settings: stored ?? { autoCreateJobs: false, resultSummaryTemplateId: null, jobWorkflowId: null, revision: 0,
...
export async function saveLaboratorySettings(client, identity, input) {
  ...
  fieldsOnly(input, ['revision', 'autoCreateJobs', 'resultSummaryTemplateId', 'jobWorkflowId', ...Object.keys(schemeColumns)]);
  ...
  const saved = await client.query(`INSERT INTO organization_laboratory_settings(organization_id,auto_create_jobs,result_summary_template_id,job_workflow_id,updated_by,${Object.values(schemeColumns).join(',')})
    VALUES($1,$2,$3,$4,$5,$7,$8,$9,$10,$11) ON CONFLICT(organization_id) DO UPDATE SET auto_create_jobs=EXCLUDED.auto_create_jobs,
  ...
  [identity.organization_id, input.autoCreateJobs, input.resultSummaryTemplateId ?? null, input.jobWorkflowId ?? null, identity.user_id, input.revision,
    ...Object.keys(schemeColumns).map((key) => schemeSettings?.[key] ?? null), schemeSettings !== null]);
