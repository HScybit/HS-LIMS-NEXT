...
export async function loadLaboratorySettings(client, identity) {
  ...
  return { settings: stored ?? { autoCreateJobs: false, selfAllocationEnabled: false, allowReceivingDateEdit: false, resultSummaryTemplateId: null, jobWorkflowId: null, dateFormat: null, datetimeFormat: null, revision: 0,
...
export async function saveLaboratorySettings(client, identity, input) {
  ...
  fieldsOnly(input, ['revision', 'autoCreateJobs', 'selfAllocationEnabled', 'allowReceivingDateEdit', 'resultSummaryTemplateId', 'jobWorkflowId', ...Object.keys(schemeColumns), 'dateFormat', 'datetimeFormat']);
  ...
  const saved = await client.query(`INSERT INTO organization_laboratory_settings(organization_id,auto_create_jobs,result_summary_template_id,job_workflow_id,updated_by,${Object.values(schemeColumns).join(',')},self_allocation_enabled,date_format,datetime_format,allow_receiving_date_edit)
    VALUES($1,$2,$3,$4,$5,$7,$8,$9,$10,$11,coalesce($13,false),$14,$15,coalesce($18,false)) ON CONFLICT(organization_id) DO UPDATE SET auto_create_jobs=EXCLUDED.auto_create_jobs,
