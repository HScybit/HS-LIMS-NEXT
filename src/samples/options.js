...
export async function sampleRegistrationOptions(client, identity) {
  ...
  const decisionRules = await rows(`SELECT rule.id, rule.name, rule.product_id AS "productId", rule.test_parameter_id AS "testParameterId", rule.method_id AS "methodId",
    coalesce(array_agg(DISTINCT category.sample_category_id) FILTER (WHERE category.sample_category_id IS NOT NULL), '{}') AS "sampleCategoryIds",
    rule.is_nabl AS "isNabl", rule.minimum_size AS "minimumSize",
    ...
    LEFT JOIN decision_rule_sample_categories category ON category.organization_id=rule.organization_id AND category.decision_rule_id=rule.id
    WHERE rule.organization_id=$1 AND rule.active
    GROUP BY rule.id, rule.name, rule.product_id, rule.test_parameter_id, rule.method_id, rule.is_nabl, rule.minimum_size, rule.estimated_time_in_days, rule.estimated_charges
    ORDER BY lower(rule.name), rule.id`);
  ...
  const { allowReceivingDateEdit, canQuickCreateCustomer } = (await client.query(`SELECT sample_receiving_date_edit_enabled() AS "allowReceivingDateEdit",
    organization_has_module_access('customer') AND app_has_permission('samples.create') AS "canQuickCreateCustomer"`)).rows[0];
  ...
  return { sampleCategories, customers: [...customerById.values()], products, tags, measurementUnits, testParameters: [...parameters.values()], decisionRules, laboratories, allowReceivingDateEdit, canQuickCreateCustomer };
}
