...
import { listCustomers } from '@/masters/customer-list.js';
...
export const GET = endpoint(async (request) => {
  ...
  return json(await authenticated(request, (client, identity) => listCustomers(client, identity, input), { readOnly: true }));
});

export const POST = endpoint(async (request) => {
  const input = await readInput(request, { maxBytes: 8 * 1_048_576 });
  return json(await authenticated(request, (client, identity) => saveCustomer(client, identity, input), { permission: 'masters.manage' }));
});
