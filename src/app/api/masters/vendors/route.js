...
import { listVendors } from '@/masters/vendor-list.js';
...
export const GET = endpoint(async (request) => {
  ...
  return json(await authenticated(request, (client, identity) => listVendors(client, identity, input), { readOnly: true }));
});

export const POST = endpoint(async (request) => {
  const input = await readInput(request, { maxBytes: 8 * 1_048_576 });
  return json(await authenticated(request, (client, identity) => saveVendor(client, identity, input), { permission: 'masters.manage' }));
});
