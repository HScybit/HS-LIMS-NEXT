...
export const GET = endpoint(async (request, context) => {
  ...
  const result = await authenticated(request, async (client, identity) => withTemplateImages(client, identity.organization_id, await loadDefinition(client, identity.organization_id, versionId)), { readOnly: true, permission: 'templates.read' });
...
export const PATCH = endpoint(async (request, context) => {
  ...
  const result = await authenticated(request, async (client, identity) => withTemplateImages(client, identity.organization_id, await editTemplate(client, identity, versionId, input.revision, input.command)), { permission: 'templates.manage' });
  return json({ model: templateView(result.model), metrics: result.metrics });
});
