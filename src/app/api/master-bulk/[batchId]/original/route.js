...
import { masterBulkOriginal } from '@/masters/bulk-export.js';
import { writeMasterXlsx } from '@/masters/bulk-xlsx-export.js';

export const GET = endpoint(async (request, context) => {
  const { batchId } = await context.params;
  const data = await authenticated(request, (client, identity) => masterBulkOriginal(client, identity, batchId), { readOnly: true });
