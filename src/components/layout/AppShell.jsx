  ...
  function navigation(mobile = false) {
    return <aside className={`lims-sidebar d-flex flex-column ${!mobile && collapsed ? 'is-collapsed' : ''}`}>
      ...
      <div className="sidebar-nav flex-grow-1">
        {identity.permissions.some((permission) => ['masters.read', 'masters.manage'].includes(permission)) ? <section className="sidebar-section"><div className="sidebar-label"><span>Master Data</span></div><div className="d-grid gap-1"><Link href="/test_parameters" className={`sidebar-link btn text-start ${pathname.startsWith('/test_parameters') ? 'is-active' : ''}`} aria-label="Test Parameters" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="list" size={20} /></span><span className="hide-menu">Test Parameters</span></Link></div></section> : null}
