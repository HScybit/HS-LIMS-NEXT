  ...
  const pageLabel = pathname.includes('/data_sheets/') ? 'Add Results' : pathname.includes('/test_requests/') ? 'Test Request'
    : pathname.startsWith('/samples') ? 'Samples' : pathname.startsWith('/master_template_management') ? 'Master Templates'
      : pathname === '/header_management' ? 'Headers' : pathname === '/footer_management' ? 'Footers'
        ...
        : pathname === '/data_transfer' ? 'Data Transfer'
  ...
  function navigation(mobile = false) {
    return <aside className={`lims-sidebar d-flex flex-column ${!mobile && collapsed ? 'is-collapsed' : ''}`}>
      ...
      <div className="sidebar-nav flex-grow-1">
        {/* Sidebar sections and their order mirror PERN's App.jsx navigation (and Meteor's sidebarModules.js): Home,
            LIMS, Instrument, Service Agreements, Customer Master, Vendor Master, Inventory, Administration and
            Platform Administration, each with only its own dedicated record links. Neither source app puts
            Products/Parameters/MoA/Sample Categories/Decision Rules/Custom Fields/Checklists/Workflow/Templates/
            Organization Settings/User/Role/Lab/Unit Management directly in the sidebar — both reach every one of
            those through the Admin Hub only. */}
        <section className="sidebar-section"><div className="sidebar-label"><span>Home</span></div><div className="d-grid gap-1"><Link href="/dashboard" className={`sidebar-link btn text-start ${pathname === '/dashboard' ? 'is-active' : ''}`} aria-label="Dashboard" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="fa-dashboard" size={20} /></span><span className="hide-menu">Dashboard</span></Link></div></section>
        {identity.permissions.some((permission) => ['samples.read', 'samples.create'].includes(permission)) ? <section className="sidebar-section"><div className="sidebar-label"><span>LIMS</span></div><div className="d-grid gap-1"><Link href={identity.permissions.includes('samples.read') ? '/samples' : '/samples/new'} className={`sidebar-link btn text-start ${pathname.startsWith('/samples') ? 'is-active' : ''}`} aria-label="Samples" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="all-samples" size={20} /></span><span className="hide-menu">All Samples</span></Link></div></section> : null}
        {identity.masterModules?.instrument ? <section className="sidebar-section"><div className="sidebar-label"><span>Instrument</span></div><div className="d-grid gap-1"><Link href="/equipments" className={`sidebar-link btn text-start ${pathname.startsWith('/equipments') ? 'is-active' : ''}`} aria-label="Instrument Management" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="tool" size={20} /></span><span className="hide-menu">Instrument Management</span></Link></div></section> : null}
        {identity.masterModules?.service_agreements ? <section className="sidebar-section"><div className="sidebar-label"><span>Service Agreements</span></div><div className="d-grid gap-1"><Link href="/service_agreements" className={`sidebar-link btn text-start ${pathname.startsWith('/service_agreements') ? 'is-active' : ''}`} aria-label="Service Agreements" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="globe" size={20} /></span><span className="hide-menu">Service Agreements</span></Link></div></section> : null}
        {identity.masterModules?.customer ? <section className="sidebar-section"><div className="sidebar-label"><span>Customer Master</span></div><div className="d-grid gap-1"><Link href="/customer_masters" className={`sidebar-link btn text-start ${pathname.startsWith('/customer_masters') ? 'is-active' : ''}`} aria-label="Customer Masters" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="globe" size={20} /></span><span className="hide-menu">Customer Masters</span></Link></div></section> : null}
        {identity.masterModules?.vendor ? <section className="sidebar-section"><div className="sidebar-label"><span>Vendor Master</span></div><div className="d-grid gap-1"><Link href="/vendor_masters" className={`sidebar-link btn text-start ${pathname.startsWith('/vendor_masters') ? 'is-active' : ''}`} aria-label="Vendors" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="globe" size={20} /></span><span className="hide-menu">Vendors</span></Link></div></section> : null}
        {identity.masterModules?.inventory ? <section className="sidebar-section"><div className="sidebar-label"><span>Inventory</span></div><div className="d-grid gap-1">
          ...
          <Link href="/material_categories" className={`sidebar-link btn text-start ${pathname.startsWith('/material_categories') ? 'is-active' : ''}`} aria-label="Material Categories" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="master-data" size={20} /></span><span className="hide-menu">Material Categories</span></Link>
        </div></section> : null}
        ...
        {identity.isPlatformAdministrator ? <section className="sidebar-section"><div className="sidebar-label"><span>Platform Administration</span></div><div className="d-grid gap-1">
          <Link href="/administration/organizations" className={`sidebar-link btn text-start ${pathname.startsWith('/administration/organizations') ? 'is-active' : ''}`} aria-label="Organizations" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="building" size={20} /></span><span className="hide-menu">Organizations</span></Link>
          <Link href="/administration/users" className={`sidebar-link btn text-start ${pathname.startsWith('/administration/users') ? 'is-active' : ''}`} aria-label="Users" onClick={() => setMobileOpen(false)}><span className="smplfy-sidebar-link-icon" aria-hidden="true"><AppIcon name="users" size={20} /></span><span className="hide-menu">Users</span></Link>
        </div></section> : null}
