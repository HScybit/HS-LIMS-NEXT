...
import { editedTextTitle, textWidgetTitle } from '../../templates/text.js';
...
export default function TextWidget({ field, mode, value, occurrenceId, disabled, onBeginEdit, onChange, onCommit }) {
  ...
  if (mode !== 'edit' || !field.editable) return <div>{title}</div>;
  ...
  return <span className="d-flex align-items-center w-100">
    <button type="button" className="border-0 bg-transparent p-0 text-body flex-grow-1 text-center" style={{ font: 'inherit' }} disabled={disabled} onClick={begin}>{title}</button>
