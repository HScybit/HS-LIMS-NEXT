...
export default function TestParameterList({ canManage, bulkResources }) {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/masters/test-parameters/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
  ...
  return <>
    ...
    <Modal open={Boolean(deletion)} title="Delete Test Parameter" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
      ...
      <p>Delete <strong>{deletion?.name}</strong>?</p><p className="mb-0 text-muted">This parameter will be removed from the list. Existing sample references and saved history will be retained.</p>
    </Modal>
  </>;
}
