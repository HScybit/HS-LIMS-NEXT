...
export default function VendorList({ canManage, canRead, bulkResources }) {
  ...
  const returnPath = `/vendor_masters${search.size ? `?${search}` : ''}`;
  const columns = useMemo(() => [
    { key: 'name', header: 'Name', searchable: true },
    { key: 'contact_person_name', header: 'Contact Person Name', searchable: true },
    { key: 'contact_person_email', header: 'Contact Person Email', searchable: true },
    { key: 'contact_person_phone', header: 'Contact Person Phone', searchable: true },
    ...
    { key: 'actions', header: 'Actions', minWidth: 160, render: (row) => <div className="d-flex flex-nowrap align-items-center gap-2">
      <Link className={actionClass} href={`/vendor_masters/${row._id}/view?from=${encodeURIComponent(returnPath)}`}><AppIcon name="eye" /><span>View</span></Link>
      {canManage ? <><Link className={actionClass} href={`/vendor_masters/${row._id}/edit?from=${encodeURIComponent(returnPath)}`}><AppIcon name="edit" /><span>Edit</span></Link>
  ...
  useEffect(() => {
    ...
    apiRequest('/api/masters/vendors/custom-fields?view=list', { signal: controller.signal }).then(({ fields }) => {
  ...
  const loadRows = useCallback(({ page, pageSize, search, filters, sort }) => {
    void reload;
    return apiRequest(`/api/masters/vendors?query=${encodeURIComponent(JSON.stringify({ page, pageSize, search, filters, sort }))}`);
  }, [reload]);
  useEffect(() => {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/masters/vendors/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
      setDeletion(null); setReload((value) => value + 1);
    } catch (failure) { setError(failure.message); }
    finally { setDeleting(false); }
  }
  if (!canRead) return <div className="container-fluid py-4"><div className="alert alert-warning" role="alert">Vendor module access is required.</div></div>;
  return <>
    <PageHeader><div className="page-header"><div className="container-fluid h-100"><div className="row h-100 align-items-center justify-content-between page-header__row">
      <div className="col page-header__start"><h1 className="page-title mb-0">Vendors</h1></div>
      <div className="col-auto page-header__actions d-flex gap-2">{canManage ? <><MasterBulkButton resource="vendors" resources={bulkResources} /><PrimaryButton leftIcon="plus" onClick={() => router.push(`/vendor_masters/new?from=${encodeURIComponent(returnPath)}`)}>New Vendor</PrimaryButton></> : null}</div>
    ...
    <Modal open={Boolean(deletion)} title="Delete Vendor" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
      ...
      <p>Delete <strong>{deletion?.name}</strong>?</p><p className="mb-0 text-muted">This Vendor will be removed from the list.</p>
    </Modal>
  </>;
}
