...
export default function ServiceAgreementForm({ agreement }) {
  ...
  return <Profiler id="service-agreement-form" onRender={(_id, phase, duration, _base, start) => performance.measure(`service-agreement:react-${phase}`, { start, duration })}>
    <FormPage title={agreement ? 'Edit Service Agreement' : 'New Service Agreement'} backTo={returnPath} backLabel="Back to service agreements"
      formId="service-agreement-form" onSubmit={save} saving={saving} disabled={blocked}
      submitLabel={agreement ? 'Update' : 'Create'}
      error={error ? <>{error}{saveUnknown ? <p className="mb-0 mt-1">Retry saving to confirm whether your changes were saved.</p> : null}</> : null}
      ...
      <FormSection title="Coverage">
        <FormField><ServiceAgreementChoice kind="vendors" id="vendor_id" label="Vendor" value={draft.vendorId} placeholder="Select Vendor"
          disabled={disabled} error={fieldErrors.vendorId} retained={agreement ? [{ id: agreement.vendorId, name: agreement.vendorName }] : undefined} onChange={value => changeValue('vendorId', value)} /></FormField>
        <FormField><ServiceAgreementChoice kind="instruments" id="equipment_ids" label="Equipment(s)" value={draft.instrumentIds} placeholder="Select Equipment(s)" multiple
          disabled={disabled} error={fieldErrors.instrumentIds} retained={agreement?.instruments} onChange={value => changeValue('instrumentIds', value)} /></FormField>
        <FormField span={12}><FormElement type="searchable-select" label="Services Included" helperText="Vendors are offered for these service types on the selected equipment."
          inputProps={{ id: 'included_services', multiple: true, options: serviceAgreementServices, value: draft.includedServices, disabled, placeholder: 'Select services',
            onChange: value => changeValue('includedServices', value) }} /></FormField>
      </FormSection>

      <FormSection title="Term and Cost">
        {[['startDate', 'start_date', 'Start Date', 'start'], ['endDate', 'end_date', 'End Date', 'end']].map(([key, id, label, word]) => <FormField key={key}>
        ...
        {[['noOfServices', 'no_of_services', 'No of Services', 'No of Services'], ['cost', 'cost', 'Cost (in Rs.)', 'Total Balance (in Rs.)']].map(([key, id, label, placeholder]) => <FormField key={key}>
        ...
        <FormField span={12}>
          <div className="smplfy-checkbox-field"><Checkbox id="in_effect" checked={draft.inEffect} ariaLabel="In Effect" disabled={disabled} onChange={value => changeValue('inEffect', value)} />
      ...
      <FormSection title="Documents and Notes" last>
        <FormField span={12}><FormElement type="textarea" label="Notes" message={fieldErrors.notes} messageTone="error"
          inputProps={{ id: 'notes', value: draft.notes, placeholder: 'Notes and Summary', rows: 5, maxLength: 10000, onChange: change('notes'), disabled }} /></FormField>
        <FormField span={12}><ServiceAgreementFile value={attachment} disabled={saving || saveUnknown} onBusy={setUploading}
          onChange={value => { setAttachment(value); setDraft(current => ({ ...current, attachmentFileId: value?.id ?? null })); }} /></FormField>
      </FormSection>
    </FormPage>
  </Profiler>;
}
