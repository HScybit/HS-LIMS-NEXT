...
export default function CustomFieldList({ canManage }) {
  ...
  const returnPath = `/project_fields${search.size ? `?${search}` : ''}`;
  const columns = useMemo(() => [
    { key: 'label', header: 'Label', searchable: true }, { key: 'description', header: 'Description', searchable: true },
    { key: 'key', header: 'Key', searchable: true }, { key: 'associated_with', header: 'Associated With', searchable: true },
    { key: 'scheme', header: 'Scheme', searchable: true },
    { key: 'data_type', header: 'Data Type', searchable: true, filterOptions: customFieldTypes,
      format: (value) => customFieldTypes.find((type) => type.value === value)?.label ?? value },
    ...
    { key: 'actions', header: 'Actions', minWidth: 160, render: (row) => <div className="d-flex flex-nowrap align-items-center gap-2">
      <Link className={actionClass} href={`/project_fields/${row._id}/view?from=${encodeURIComponent(returnPath)}`}><AppIcon name="eye" /><span>View</span></Link>
      {canManage ? <><Link className={actionClass} href={`/project_fields/${row._id}/edit?from=${encodeURIComponent(returnPath)}`}><AppIcon name="edit" /><span>Edit</span></Link>
  ...
  const loadRows = useCallback(({ page, pageSize, search, filters, sort }) => {
    void reload;
    return apiRequest(`/api/masters/project-fields?query=${encodeURIComponent(JSON.stringify({ page, pageSize, search, filters, sort }))}`);
  }, [reload]);
  useEffect(() => {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/masters/project-fields/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
  ...
  return <>
    <PageHeader><div className="page-header"><div className="container-fluid h-100"><div className="row h-100 align-items-center justify-content-between page-header__row">
      <div className="col page-header__start"><h1 className="page-title mb-0">Custom Fields</h1></div>
      <div className="col-auto page-header__actions">{canManage ? <PrimaryButton leftIcon="plus" onClick={() => router.push(`/project_fields/new?from=${encodeURIComponent(returnPath)}`)}>New Project Field</PrimaryButton> : null}</div>
    </div></div></div></PageHeader>
    <DataTable columns={columns} loadRows={loadRows} />
    <Modal open={Boolean(deletion)} title="Delete Project Field" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
      ...
      <p>Delete <strong>{deletion?.label}</strong>?</p><p className="mb-0 text-muted">This custom field will be removed from the list.</p>
    </Modal>
  </>;
}
