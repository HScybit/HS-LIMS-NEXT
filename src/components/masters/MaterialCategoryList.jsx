...
export default function MaterialCategoryList({ canManage }) {
  ...
  const returnPath = `/material_categories${search.size ? `?${search}` : ''}`;
  const columns = useMemo(() => [
    { key: 'name', header: 'Name', searchable: true }, { key: 'description', header: 'Description', searchable: true },
    { key: 'reusable', header: 'Reusable', type: 'boolean', format: value => value ? 'Yes' : 'No' },
    { key: 'expirable', header: 'Expirable', type: 'boolean', format: value => value ? 'Yes' : 'No' },
    ...
    { key: 'actions', header: 'Actions', minWidth: 160, render: (row) => <div className="d-flex flex-nowrap align-items-center gap-2">
      <Link className={actionClass} href={`/material_categories/${row._id}/view?from=${encodeURIComponent(returnPath)}`}><AppIcon name="eye" /><span>View</span></Link>
      {canManage ? <><Link className={actionClass} href={`/material_categories/${row._id}/edit?from=${encodeURIComponent(returnPath)}`}><AppIcon name="edit" /><span>Edit</span></Link>
  ...
  const loadRows = useCallback(({ page, pageSize, search, filters, sort }) => {
    void reload;
    return apiRequest(`/api/masters/material-categories?query=${encodeURIComponent(JSON.stringify({ page, pageSize, search, filters, sort }))}`);
  }, [reload]);
  useEffect(() => {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/masters/material-categories/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
  ...
  return <>
    <PageHeader><div className="page-header"><div className="container-fluid h-100"><div className="row h-100 align-items-center justify-content-between page-header__row">
      <div className="col page-header__start"><h1 className="page-title mb-0">Material Categories</h1></div>
      <div className="col-auto page-header__actions">{canManage ? <PrimaryButton leftIcon="plus" onClick={() => router.push(`/material_categories/new?from=${encodeURIComponent(returnPath)}`)}>New Material Category</PrimaryButton> : null}</div>
    </div></div></div></PageHeader>
    <DataTable columns={columns} loadRows={loadRows} />
    <Modal open={Boolean(deletion)} title="Delete Material Category" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
      ...
      <p>Delete <strong>{deletion?.name}</strong>?</p><p className="mb-0 text-muted">This category will be removed from the list. Its saved history will be retained.</p>
    </Modal>
  </>;
}
