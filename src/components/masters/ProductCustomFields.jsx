...
import ProductCustomFieldFile from './ProductCustomFieldFile.jsx';
...
function Field({ field, value, stored, disabled, error, onChange, onBusy, userOptions, loadUsers }) {
  const control = customFieldControl(field); const id = `product-custom-field-${field.id}`;
  if (control.type === 'file') return <ProductCustomFieldFile {...{ field, value, stored, disabled, error, onChange, onBusy }} />;
...
export default function ProductCustomFields({ fields, loading, loadError, values, storedFields = [], errors, disabled, generatingId, onChange, onBusy, onGenerate, onReload }) {
  ...
  const loadUsers = useCallback((search) => {
    ...
    const pending = apiRequest(`/api/masters/products/custom-field-users?search=${encodeURIComponent(search)}`, { signal: abort.signal })
      .then((result) => {
  ...
  return <Profiler id="product-custom-fields" onRender={(_id, phase, duration, _base, start) => performance.measure(`product-fields:react-${phase}`, { start, duration })}>
    ...
    {fields.map((field) => {
      const content = <Field field={field} value={values[field.id]} stored={storedById.get(field.id)} disabled={disabled} error={errors[field.id]}
