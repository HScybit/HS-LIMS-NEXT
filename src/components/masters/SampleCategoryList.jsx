...
export default function SampleCategoryList({ canManage }) {
  ...
  const returnPath = `/sample_categories${search.size ? `?${search}` : ''}`;
  const columns = useMemo(() => [
    { key: 'name', header: 'Name', searchable: true }, { key: 'description', header: 'Description', searchable: true },
    { key: 'retentionDays', header: 'Retention Days' }, { key: 'estimatedTimeInDays', header: 'Estimated Days' },
    { key: 'workflowName', header: 'Workflow' }, { key: 'createdAt', header: 'Created At', type: 'date' },
    { key: 'actions', header: 'Actions', minWidth: 160, render: (row) => <div className="d-flex flex-nowrap align-items-center gap-2">
      <Link className={actionClass} href={`/sample_categories/${row._id}/view?from=${encodeURIComponent(returnPath)}`}><AppIcon name="eye" /><span>View</span></Link>
      {canManage ? <><Link className={actionClass} href={`/sample_categories/${row._id}/edit?from=${encodeURIComponent(returnPath)}`}><AppIcon name="edit" /><span>Edit</span></Link>
  ...
  const loadRows = useCallback(({ page, pageSize, search }) => {
    void reload;
    return apiRequest(`/api/masters/sample-categories?query=${encodeURIComponent(JSON.stringify({ page, pageSize, search }))}`);
  }, [reload]);
  useEffect(() => {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/masters/sample-categories/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
  ...
  return <>
    <PageHeader><div className="page-header"><div className="container-fluid h-100"><div className="row h-100 align-items-center justify-content-between page-header__row">
      <div className="col page-header__start"><h1 className="page-title mb-0">Sample Categories</h1></div>
      <div className="col-auto page-header__actions">{canManage ? <PrimaryButton leftIcon="plus" onClick={() => router.push(`/sample_categories/new?from=${encodeURIComponent(returnPath)}`)}>New Sample Category</PrimaryButton> : null}</div>
    </div></div></div></PageHeader>
    <DataTable columns={columns} loadRows={loadRows} />
    <Modal open={Boolean(deletion)} title="Delete Sample Category" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
