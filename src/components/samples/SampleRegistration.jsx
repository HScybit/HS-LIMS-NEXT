    ...
    <div className="sample-form-page min-vh-100 bg-body-tertiary"><main className="sample-form-page__body px-4 py-4 pb-5">
      ...
      <form id={formId} className="container-xl px-0" onSubmit={submit}><div className="d-grid gap-3">
        ...
        <SampleFormSection id="new-sample-additional-details" title="Additional Details"><div className="container-fluid p-4"><div className="row g-4">
          ...
          {/* Root cause/CAPA are investigation output that cannot exist before a complaint is registered. */}
          {form.kind === 'complaint' && editing ? <div className="col-12">{textField('complaintRca', 'Root Cause', { textarea: true, rows: 3, maxLength: 5000 })}</div> : null}
          {form.kind === 'complaint' && editing ? <div className="col-12">{textField('complaintCapa', 'Corrective/Preventive Action', { textarea: true, rows: 3, maxLength: 5000 })}</div> : null}
        </div></div></SampleFormSection>
      </div></form>
    </main></div>
