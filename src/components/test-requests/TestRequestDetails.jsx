...
export default function TestRequestDetails({ requestId, sampleId }) {
    ...
    async function load() {
      try {
        ...
        const [runtime, workflow] = await Promise.all([
          request.datasheetId ? apiRequest(`/api/datasheets/${request.datasheetId}?sampleId=${encodeURIComponent(sampleId)}`, { signal: controller.signal }) : null,
          request.workflowRunId ? apiRequest(`/api/workflow-runs/${request.workflowRunId}`, { signal: controller.signal }) : null,
        ]);
        if (!controller.signal.aborted) { setData({ request, runtime, workflow }); setError(''); }
  ...
  return <>
    ...
    <main className="smplfy-tr-details-page bg-body-tertiary min-vh-100"><div className="row g-3 align-items-stretch"><section className="col-12 col-xl min-w-0"><div className="tr-details-workspace">
      ...
      workflow={workflow} runtime={runtime} currentState={request.stateName || status?.[0] || request.status} onChanged={() => setReload((value) => value + 1)} /></div>
    </div></main>
  </>;
}
