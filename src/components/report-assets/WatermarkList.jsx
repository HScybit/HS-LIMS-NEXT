...
export default function WatermarkList({ canManage }) {
  ...
  const returnPath = `/watermark_report${search.size ? `?${search}` : ''}`;
  const columns = useMemo(() => [
    { key: 'name', header: 'Name', searchable: true }, { key: 'created_at', header: 'Created At', type: 'date' },
    { key: 'actions', header: 'Actions', minWidth: 160, render: (row) => <div className="d-flex flex-nowrap align-items-center gap-2">
      <Link className={actionClass} href={`/watermark_report/${row._id}/view?from=${encodeURIComponent(returnPath)}`}><AppIcon name="eye" /><span>View</span></Link>
      {canManage ? <><Link className={actionClass} href={`/watermark_report/${row._id}/edit?from=${encodeURIComponent(returnPath)}`}><AppIcon name="edit" /><span>Edit</span></Link>
  ...
  const loadRows = useCallback(({ page, pageSize, search, filters, sort }) => {
    // Changing the refresh revision asks the existing DataTable to fetch again.
    void reload;
    return apiRequest(`/api/report-assets/watermarks?query=${encodeURIComponent(JSON.stringify({ page, pageSize, search, filters, sort }))}`);
  }, [reload]);
  useEffect(() => {
  ...
  async function remove() {
    ...
    try {
      await apiRequest(`/api/report-assets/watermarks/${deletion._id}`, { method: 'DELETE', body: { requestId: deletion.requestId, revision: deletion.revision } });
  ...
  return <>
    <PageHeader><div className="page-header"><div className="container-fluid h-100"><div className="row h-100 align-items-center justify-content-between page-header__row">
      <div className="col page-header__start"><h1 className="page-title mb-0">Watermark Report</h1></div>
      <div className="col-auto page-header__actions">{canManage ? <PrimaryButton leftIcon="plus" onClick={() => router.push(`/watermark_report/new?from=${encodeURIComponent(returnPath)}`)}>New Watermark Report</PrimaryButton> : null}</div>
    </div></div></div></PageHeader>
    <DataTable columns={columns} loadRows={loadRows} />
    <Modal open={Boolean(deletion)} title="Delete Watermark Report" onClose={() => { if (!deleting) setDeletion(null); }} actions={<>
      ...
      <p>Delete <strong>{deletion?.name}</strong>?</p><p className="mb-0 text-muted">This watermark will be removed from the list. Its saved history and images will be retained.</p>
    </Modal>
  </>;
}
