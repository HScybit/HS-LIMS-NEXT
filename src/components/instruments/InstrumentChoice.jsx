...
const empty = [];
export default function InstrumentChoice({ kind, id, label, value, onChange, multiple = false, required = false, disabled, error, placeholder, retained = empty }) {
  ...
  const controllers = useRef(new Set()); const requests = useRef(new Map());
  const selected = JSON.stringify(multiple ? value : value ? [value] : []);
  const load = useCallback(search => {
    if (requests.current.has(search)) return requests.current.get(search);
    const controller = new AbortController(); controllers.current.add(controller);
    const query = { kind, search, selectedIds: JSON.parse(selected) };
    const pending = apiRequest('/api/instruments/options', { method: 'POST', body: query, signal: controller.signal }).then(result => {
  ...
  return <div><FormElement type="searchable-select" label={label} mandatory={required} message={error || failure} messageTone="error"
    inputProps={{ id, value, options, defaultOptions: initial, loadOptions: load, cacheOptions: false, multiple, clearable: !required,
