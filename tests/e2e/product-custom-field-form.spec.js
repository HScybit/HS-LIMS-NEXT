...
test('Product source controls capture every type, retry lost upload/save responses and retain historical attachment bytes',async({page},testInfo)=>{
  ...
  // RelationSelect searches the displayed label; removed raw separators do not match.
  ...
  await expect(page.getByRole('listbox').getByRole('option',{name:'Synthetic Analyst',exact:true})).toHaveCount(0);
  await page.getByLabel('Custom Users',{exact:true}).fill('Synthetic Analyst');
