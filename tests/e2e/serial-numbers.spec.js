...
test('designer serial numbers reflect padding, row order and reload', async ({ page }, testInfo) => {
  ...
  await field.getByLabel('0 Padding', { exact: true }).fill('3');
  ...
  await page.locator(`.widget-row[data-row-id="${template.records.rows[0].id}"] .template-studio-node-identity`).click();
  await dock.getByRole('button', { name: 'Move down', exact: true }).click();
  ...
  await column.locator('.template-studio-field-action').click();
  await field.getByLabel('0 Padding', { exact: true }).fill('');
  await field.getByRole('button', { name: 'Save field', exact: true }).click(); await expect(page.getByText('Updated successfully!').last()).toBeVisible();
