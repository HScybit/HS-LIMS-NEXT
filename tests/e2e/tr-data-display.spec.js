...
test('TR Data renders scientific markup in datasheets and frozen reports while retaining the source designer preview', async ({ page }, testInfo) => {
  ...
  const column = flow.template.records.columns.find((column) => column.id === reportField.columnId);
  const sectionId = flow.template.records.rows.find((row) => row.id === column.rowId).sectionId;
  await page.locator(`[data-section-id="${sectionId}"]`).getByRole('button', { name: 'Edit Mode Off', exact: true }).click();
