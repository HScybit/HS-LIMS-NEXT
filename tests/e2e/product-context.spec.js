...
test('Product Title/Key controls, distinct report lines and frozen nested values survive edits and print through the worker', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#sampleify-toast-shelf').getByText('Key already exist!', { exact: true })).toBeVisible();
  ...
  const configured = page.waitForResponse((response) => response.url().endsWith(`/api/template-versions/${flow.template.versionId}`) && response.request().method() === 'PATCH');
