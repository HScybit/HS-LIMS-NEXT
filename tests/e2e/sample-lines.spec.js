...
test('the source line attribute selector preserves separate configuration and retries a failed choice', async ({ page }, testInfo) => {
  ...
  await settings.getByRole('button', { name: 'Save', exact: true }).click(); await expect(page.getByText('Updated successfully!').last()).toBeVisible();
  await column.locator('.template-studio-field-action').click();
  const editor = page.getByRole('dialog', { name: 'Configure field' });
  await editor.getByLabel('Key', { exact: true }).fill('line_description');
  await editor.getByLabel('Attribute Name/Key', { exact: true }).fill('Unused independent attribute');
  await editor.getByLabel('Default Value', { exact: true }).fill('Unused configured default');
  const fields = editor.locator('.template-designer-form-section').filter({ hasText: 'Identifiers must be unique across the template.' });
  await expect(fields.getByLabel('Title', { exact: true })).toHaveCount(0);
  await expect(fields.getByRole('combobox')).toHaveCount(0);
  await editor.getByRole('button', { name: 'Save field', exact: true }).click(); await expect(page.getByText('Updated successfully!').last()).toBeVisible();
