...
async function fixture(permissions = ['instruments.manage']) {
  ...
  const services = [{ id: randomUUID(), serviceCode: 'calibration', displayLabel: 'Calibration', isActive: true },
    { id: randomUUID(), serviceCode: 'inspection', displayLabel: 'Inspection', isActive: true }];
  await saveModuleAccessSettings(setup, modules, { instrumentServiceTypes: services });
  ...
  return { setup, actor, modules, services, laboratoryId };
}
...
async function additional(page) {
  for (let step = 0; step < 3 && await page.getByRole('button', { name: 'Next', exact: true }).count(); step++) await page.getByRole('button', { name: 'Next', exact: true }).click();
...
test('Instrument wizard, source columns, filters and QR retain exact lost-save and deletion requests', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('tab', { name: 'Inspection', exact: true })).toBeVisible();
...
test('Instrument edits preserve hidden inactive metadata and drafts after stale saves and replace inactive service types', async ({ page }) => {
  ...
  const record = await seed(context, { active: false, calibrated: false, costOfEquipment: '0', currentLocation: 'Hidden location', manufacturerSupplier: 'Hidden supplier',
    serviceConfigurations: context.services.map(service => ({ id: randomUUID(), serviceCode: service.serviceCode, reminderBeforeDays: 0 })) });
  context.services[1].isActive = false; await saveModuleAccessSettings(context.setup, context.modules, { instrumentServiceTypes: context.services });
  await login(page, context.actor); await page.goto('/equipments/' + record.id + '/edit');
  await expect(page.getByRole('button', { name: 'Inspection Details', exact: true })).toHaveCount(0);
  ...
  expect(saved.serviceConfigurations).toHaveLength(1); expect(saved.serviceConfigurations[0].id).toBe(record.serviceConfigurations[0].id);
...
test('Instrument read-only, record restrictions and module revocations apply to pages, navigation and HTTP', async ({ page }) => {
  ...
  context.modules[2].userIds.push(reader.userId); await saveModuleAccessSettings(context.setup, context.modules, { instrumentServiceTypes: context.services });
  ...
  context.modules[2].userIds = [context.actor.userId]; await saveModuleAccessSettings(context.setup, context.modules, { instrumentServiceTypes: context.services });
...
test('all 100 configured services and Additional Details remain reachable in the desktop wizard', async ({ page }) => {
  const context = await fixture();
  const services = Array.from({ length: 100 }, (_, index) => ({ id: randomUUID(), serviceCode: 'service_' + index, displayLabel: 'Service ' + index, isActive: true }));
  await saveModuleAccessSettings(context.setup, context.modules, { instrumentServiceTypes: services });
  const record = await seed(context, { serviceConfigurations: services.map(service => ({ id: randomUUID(), serviceCode: service.serviceCode })) });
  await page.setViewportSize({ width: 1440, height: 960 }); await login(page, context.actor);
  await page.goto('/equipments/' + record.id + '/edit');
  const sidebar = page.locator('.smplfy-new-instrument-page aside');
  await expect(sidebar.getByRole('button')).toHaveCount(102);
  await sidebar.hover(); await page.mouse.wheel(0, 15000);
  const additional = sidebar.getByRole('button', { name: 'Additional Details', exact: true });
  await expect(additional).toBeInViewport(); await additional.click();
  const saved = page.waitForResponse(response => response.url().endsWith('/api/instruments') && response.request().method() === 'POST');
  await page.getByRole('button', { name: 'Save Changes', exact: true }).click();
  expect((await (await saved).json()).serviceConfigurations).toHaveLength(100);
});
