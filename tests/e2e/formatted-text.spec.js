...
test('formatted titles retain raw editing, scientific markup, safe fallback and frozen history in the designer and datasheet', async ({ page, context }, testInfo) => {
  ...
  await expect(titleCells.locator('.row1 strong')).toHaveText('Water H2O'); await expect(titleCells.locator('sub')).toHaveText('2'); await expect(titleCells.locator('sup')).toHaveText('2');
  await expect(tableCells.locator('td')).toHaveText('Table 0'); await expect(tableCells.locator('td')).toHaveCSS('text-align', 'center');
  await expect(page.locator(`[data-field-id="${fields.unbound_title}"] .row1`)).toHaveText(unbound); await expect(page.locator(`[data-field-id="${fields.unbound_title}"] img`)).toHaveCount(0);
  await titleCells.locator('.template-studio-field-action').click();
  const field = page.getByRole('dialog', { name: 'Configure field' });
  await expect(field.getByLabel('Title', { exact: true })).toHaveValue(title);
  await field.getByRole('button', { name: 'Save field', exact: true }).click(); await expect(page.getByText('Updated successfully!').last()).toBeVisible();
