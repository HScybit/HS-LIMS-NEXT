...
test('source image chooser, layout controls, animation and fixed print frame survive a lost response and reload', async ({ page }, testInfo) => {
  ...
  await page.locator('.widget-col .template-studio-field-action').click();
  ...
  const configured = page.waitForResponse((response) => response.url().endsWith(`/api/template-versions/${template.versionId}`) && response.request().method() === 'PATCH');
  await dialog.getByRole('button', { name: 'Save field' }).click(); expect((await configured).status()).toBe(200);
  await expect(image.locator('..').locator('..')).toHaveCSS('margin-right', '-2.5px');
  await page.getByRole('group', { name: 'Designer mode' }).getByRole('button', { name: 'View' }).click(); await expect(page.locator('input[type=file]')).toHaveCount(0);
...
test('uploading in a frozen designer creates a draft and keeps existing capture images unchanged', async ({ page, context }) => {
  ...
  for (const tab of [page, other]) await expect(tab.locator('img.tiw_image')).toBeVisible();
