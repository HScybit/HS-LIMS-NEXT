...
test('TR Data Title, Key, Required and Default Value retain source controls through errors and reload', async ({ page }, testInfo) => {
  ...
  const section = page.locator(`[data-section-id="${row.sectionId}"]`); const column = page.locator(`[data-field-id="${field.id}"]`);
  const dialog = page.getByRole('dialog', { name: 'Widget Configuration', exact: true });
  async function open() {
    await section.getByRole('button', { name: 'Edit Mode Off', exact: true }).click();
  ...
  await dialog.getByRole('button', { name: 'Update data', exact: true }).click(); await expect(page.getByText('Key already exist!', { exact: true })).toBeVisible();
  ...
  await dialog.getByRole('button', { name: 'Update data', exact: true }).click(); await expect(page.getByText('Synthetic TR configuration save failure', { exact: true })).toBeVisible();
  ...
  await dialog.getByRole('button', { name: 'Update data', exact: true }).click(); expect((await saved).status()).toBe(200); await expect(dialog).toBeHidden();
  ...
  await dialog.getByLabel('Default Value', { exact: true }).fill('-'); await dialog.getByRole('button', { name: 'Update data', exact: true }).click(); await expect(dialog).toBeHidden();
