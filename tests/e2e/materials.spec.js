...
async function fixture({ expirable = true, create = true, initialQuantity = '0' } = {}) {
  const account = await createAccount(owner, { permissions: ['masters.manage'] }); const session = await signIn({ identifier: account.username, password: account.password });
...
test('stale material edits and lookup failures keep the draft, while read-only and foreign users remain restricted', async ({ page }) => {
  ...
  const reader = await createAccount(owner, { organizationId: data.account.organizationId, permissions: ['masters.read'] }); await login(page, reader); await page.goto('/materials');
