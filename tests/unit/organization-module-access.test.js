...
test('Service Agreement settings require the complete supported prefix and preserve older caller shapes', () => {
  const modules = ['customer', 'vendor', 'instrument', 'service_agreements', 'inventory'].map(moduleKey => ({ moduleKey, enabled: false, roleIds: [], userIds: [] }));
  ...
  for (const rows of [[modules[0], modules[1], modules[3]], [modules[0], modules[2], modules[3]],
    [modules[0], modules[1], modules[3], modules[3]], [...modules, modules[3]], [...modules.slice(0, 4), modules[4], modules[4]]]) {
...
const fourModules = () => ({ moduleAccess: [...settings().moduleAccess,
  { moduleKey: 'instrument', enabled: false, roleIds: [], userIds: [] },
  { moduleKey: 'service_agreements', enabled: false, roleIds: [], userIds: [] }] });

test('Inventory access is explicit and optional only for older four-module callers', () => {
  const legacy = fourModules(); assert.deepEqual(moduleAccessSettingsInput(legacy), legacy.moduleAccess);
  const inventory = { moduleKey: 'inventory', enabled: true, roleIds: [randomUUID()], userIds: [randomUUID()] };
  const input = { moduleAccess: [...legacy.moduleAccess, inventory] }; const before = structuredClone(input);
  assert.deepEqual(moduleAccessSettingsInput(input), [...legacy.moduleAccess, inventory]);
  assert.deepEqual(input, before);
  invalid(() => moduleAccessSettingsInput({ moduleAccess: [legacy.moduleAccess[0], inventory] }));
  invalid(() => moduleAccessSettingsInput({ moduleAccess: [...legacy.moduleAccess, inventory, inventory] }));
  const sparse = [...legacy.moduleAccess, inventory]; delete sparse[3];
  invalid(() => moduleAccessSettingsInput({ moduleAccess: sparse }));
});

test('Inventory selections preserve disabled access and enforce their own limits', () => {
  const ids = Array.from({ length: 501 }, () => randomUUID());
  const inventory = { moduleKey: 'inventory', enabled: false, roleIds: ids.slice(0, 500), userIds: ids.slice(0, 500) };
  const input = { moduleAccess: [...fourModules().moduleAccess, inventory] };
  assert.deepEqual(moduleAccessSettingsInput(input)[4], inventory);
  for (const key of ['roleIds', 'userIds']) {
    invalid(() => moduleAccessSettingsInput({ moduleAccess: [...fourModules().moduleAccess, { ...inventory, [key]: ids }] }));
    invalid(() => moduleAccessSettingsInput({ moduleAccess: [...fourModules().moduleAccess, { ...inventory, [key]: [ids[0], ids[0].toUpperCase()] }] }));
  }
});
