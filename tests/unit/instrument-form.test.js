...
const type = { id: randomUUID(), serviceCode: 'inspection', label: 'Inspection' };
...
test('Instrument wizard submission preserves hidden metadata, zero, false, active types and stable service identities', () => {
  const prior = { ...valid, active: false, calibrated: false, costOfEquipment: '0.00', currentLocation: 'Stored shelf', serviceConfigurations: [
    { id: randomUUID(), serviceDefinitionId: type.id, serviceCode: type.serviceCode, frequencyDays: 30, reminderBeforeDays: 0, lastPerformedOn: '2026-09-01', reminderRoleIds: [] },
    { id: randomUUID(), serviceDefinitionId: randomUUID(), serviceCode: 'retired_service' },
  ] };
...
test('Instrument wizard validates service pairs, role counts and integer boundaries and recognizes configured breakdown keys', () => {
  ...
  assert.deepEqual(instrumentFormErrors(draft), {}); assert.equal(isBreakdownService('External_Breakdown'), true); assert.equal(isBreakdownService('Inspection'), false);
});
