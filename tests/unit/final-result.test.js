...
test('numeric-prefix text and precision differences remain unresolved rather than choosing a source policy', () => {
  const data = fixture();
  ...
  for (const textValue of ['5 mg/L', '1e3', '+2', '9007199254740993', ' padded ']) {
    value.textValue = textValue;
    assert.throws(() => resolveFinalResult(textModel, data.occurrences, data.values), { code: 'scientific_policy_unresolved' });
  }
...
test('required and report errors stay explicit, optional unrelated errors do not blanket-block submission', () => {
  ...
  assert.throws(() => validateSubmissionValues(data.model, { values: data.values }, data.calculation), { code: 'scientific_policy_unresolved' });
  data.model.columnsById[data.records.columns[1].id].isFinalResult = true;
  assert.throws(() => resolveFinalResult(data.model, data.occurrences, data.values), { code: 'scientific_policy_unresolved' });
});
