...
test('qr_code_widget encodes the report sample number, falling back to the field label or key, and is empty without a report', () => {
  const report = { sample: { sampleNumber: 'SYN-001' } };
  assert.equal(contextWidgetValue(field('qr_code_widget'), report), 'SYN-001');
  assert.equal(contextWidgetValue({ widget: 'qr_code_widget', label: 'Configured label' }, { sample: {} }), 'Configured label');
  assert.equal(contextWidgetValue({ widget: 'qr_code_widget', alias: 'field_key' }, { sample: {} }), 'field_key');
  assert.equal(contextWidgetValue(field('qr_code_widget'), { sample: {} }), '');
  assert.equal(contextWidgetValue(field('qr_code_widget'), null), '');
});
