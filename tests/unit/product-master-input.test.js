...
test('product input rejects hidden links, malformed identities and invalid text or revisions', () => {
  for (const changes of [{ active: false }, { name: null }, { name: 'x'.repeat(201) },
...
test('product sample category selection is omitted by default, and rejects duplicates or oversized lists when provided', () => {
  const absent = productInput(input());
  assert.equal(absent.sampleCategoryIdsProvided, false); assert.equal(absent.sampleCategoryIds, undefined);
  const categories = Array.from({ length: 500 }, () => randomUUID());
  const provided = productInput(input({ sampleCategoryIds: categories }));
  assert.equal(provided.sampleCategoryIdsProvided, true); assert.deepEqual(provided.sampleCategoryIds, categories);
  const cleared = productInput(input({ sampleCategoryIds: [] }));
  assert.equal(cleared.sampleCategoryIdsProvided, true); assert.deepEqual(cleared.sampleCategoryIds, []);
  for (const sampleCategoryIds of [null, {}, '', [...categories, randomUUID()], [categories[0], categories[0].toUpperCase()]]) {
    assert.throws(() => productInput(input({ sampleCategoryIds })), { code: 'invalid_product_sample_categories' });
  }
  assert.throws(() => productInput(input({ sampleCategoryIds: ['missing'] })), { code: 'invalid_id' });
});
