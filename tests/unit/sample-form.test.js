...
function fixture() {
  ...
  const options = { sampleCategories: [{ id: categoryId, estimatedTimeInDays: 4 }], testParameters: [{ id: parameterId, methods: [{ id: methodId, isDefault: true }] }],
    decisionRules: [{ id: randomUUID(), name: 'Specific rule', productId, testParameterId: parameterId, methodId, sampleCategoryIds: [categoryId], estimatedTimeInDays: '2.5', estimatedCharges: '0', minimumSize: '10 mL', isNabl: false }] };
...
test('routed PERN method defaults use applicable criteria and favor method/category specificity', () => {
  ...
  const generic = { ...options.decisionRules[0], id: randomUUID(), name: 'A generic criterion', sampleCategoryIds: [], estimatedCharges: '90' };
