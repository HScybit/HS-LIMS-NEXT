...
export const emptyModuleAccess = () => ['customer', 'vendor', 'instrument', 'service_agreements', 'inventory'].map(moduleKey => ({ moduleKey, enabled: false, roleIds: [], userIds: [] }));
...
export async function grantSyntheticInventoryAccess(owner, account) {
  const manager = await createAccount(owner, { organizationId: account.organizationId, permissions: ['settings.manage'] });
  Object.assign(manager, await signIn({ identifier: manager.username, password: manager.password }));
  const modules = (await withSession(manager.token, loadLaboratorySettings, { readOnly: true })).settings.moduleAccess;
  const inventory = modules.find(access => access.moduleKey === 'inventory');
  inventory.enabled = true; inventory.userIds = [...new Set([...inventory.userIds, account.userId])];
  await saveModuleAccessSettings(manager, modules);
  return manager;
}
