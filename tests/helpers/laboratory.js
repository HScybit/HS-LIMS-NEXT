...
export async function createLaboratoryFixture(owner, account, options = {}) {
  ...
  try {
    ...
    const [category] = await db.insert(masters.sampleCategories).values({ organizationId, code: options.categoryCode ?? code(), name: 'Synthetic water', abbreviation: 'SYN-W', retentionDays: 30, estimatedTimeInDays: 2,
      enableReissue: options.enableReissue ?? false }).returning();
    ...
    const [rule] = await db.insert(masters.decisionRules).values({ organizationId, code: code(), name: 'Synthetic acceptance criterion', productId: product.id,
      testParameterId: parameter.id, methodId: method.id, templateId: template.templateId, cutoffValue: '10', lessThanText: 'Within synthetic limit', greaterThanText: 'Above synthetic limit' }).returning();
    await db.insert(masters.decisionRuleSampleCategories).values({ organizationId, decisionRuleId: rule.id, sampleCategoryId: category.id });
    ...
    if (options.workflow !== false) for (const appliesTo of ['sample', 'test_request']) {
      ...
      const [state] = await db.insert(workflowStates).values({ organizationId, workflowVersionId: version.id, code: 'initial', name: 'In Progress', stateType: 'initial',
        ...
        generateTestRequests: appliesTo === 'sample' && Boolean(options.generateTestRequests),
        showSampleReissue: appliesTo === 'sample' && Boolean(options.showSampleReissue) }).returning();
