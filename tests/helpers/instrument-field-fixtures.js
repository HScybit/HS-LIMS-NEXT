...
export async function createInstrumentFieldFixture(work, { fieldCount, instrumentCount, userId, laboratoryId, serviceTypes }) {
  const fields = await work(async (client, identity) => {
    ...
    for (let index = 0; index < fieldCount; index++) {
      ...
      result.push(await saveCustomField(client, identity, {
        ...
        associatedWith: 'instrument', fieldType: generated ? 'text' : type, displayOrder: index,
  ...
  const instrument = { name: 'Synthetic performance Instrument', laboratoryId, dateOfInstallation: '2026-09-17', allowedUserIds: [userId],
    costOfEquipment: '0', calibrated: false, serviceConfigurations: serviceTypes.map(type => ({ id: randomUUID(), serviceCode: type.serviceCode,
      lastPerformedOn: '2026-09-01', frequencyDays: 30, reminderBeforeDays: 0 })) };
  const instruments = [];
  for (let index = 0; index < instrumentCount; index++) instruments.push(await work((client, identity) => saveInstrumentCore(client, identity, {
    ...instrument, code: `I-${index}`, name: `Synthetic performance Instrument ${index}`, id: randomUUID(), requestId: randomUUID(), revision: 0,
    customFields: values, customFieldTimeZone: 'UTC',
  })));
  return { fields, values, instrument, instruments };
}
