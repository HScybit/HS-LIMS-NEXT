...
async function addTitles(client, identity, template, sectionId, rowId, { datasheet = false, titles = parameterTitleSelectors, literalTitle = 'name' } = {}) {
  ...
  const fields = {};
  for (const [alias, label, widget, editable] of titles) {
  ...
  await edit({ type: 'configureField', columnId: literalRow.columnIds[0], widget: 'text_widget', alias: 'literal_name_title', label: literalTitle });
...
export async function prepareParameterTitleFlow(owner, account, { complete = true, history = true, productLines = 1, prepareParameter, titles, literalTitle } = {}) {
  ...
  const flow = await prepareReportFlow(owner, account, {
    ...
    prepareProduct: history ? async (client, identity, fixture) => {
      parameterCommand = { id: fixture.parameter.id, revision: 1, requestId: randomUUID(),
        ...
        ...(prepareParameter ? await prepareParameter(client, identity, fixture) : {}) };
      await saveTestParameter(client, identity, parameterCommand);
    } : undefined,
    prepareDatasheet: async (client, identity, template) => {
      datasheetTitles = await addTitles(client, identity, template, template.records.sections[0].id, template.records.rows[0].id, { datasheet: true, titles, literalTitle });
    },
  });
  ...
  const reportTitles = await withSession(account.token, (client, identity) => addTitles(client, identity, flow.template, section.id, row.id, { titles, literalTitle }), { csrfToken: account.csrfToken });
