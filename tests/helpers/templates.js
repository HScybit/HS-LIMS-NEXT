...
export function analyticalRecords({ rowCount = 2, repeated = true, chain = false } = {}) {
  ...
  for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
    ...
    records.expressions.push({ id: randomUUID(), fieldId: calculatedId, purpose: 'calculate', ...parseExpression(formula, (alias) => references[alias]) });
  ...
  if (repeated && rowCount > 1) {
    ...
    Object.assign(expression, parseExpression(`SUM(${firstRaw.alias})`, () => ({ fieldId: firstRaw.id, scope: 'descendants' })));
