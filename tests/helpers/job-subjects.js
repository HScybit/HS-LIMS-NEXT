...
export async function prepareSubjectJob(owner, creator, analyst, { manualParent = false, resultWidget = false, resultValueType = 'numeric', resultDefaultValue, finalSection = false, jobWorkflowId = null, prepareTemplate, legacyChildren = false, reviewerUserId, duplicateParameterMethod = false } = {}) {
  ...
  try {
    ...
    if (duplicateParameterMethod) {
      // Q6: a job can carry two test requests for the SAME parameter (here via a second method) —
      // the datasheet must keep them as distinct rows rather than grouping/collapsing them.
      const alternateMethod = (await client.query(`INSERT INTO methods_of_analysis(organization_id,code,name,method_uuid,decimal_scale,parse_number,active)
        VALUES($1,$2,'Synthetic duplicate-parameter method',$2,3,false,true) RETURNING id`, [creator.organizationId, randomUUID()])).rows[0];
      await client.query('INSERT INTO parameter_methods(organization_id,test_parameter_id,method_id) VALUES($1,$2,$3)',
        [creator.organizationId, source.parameter.id, alternateMethod.id]);
      source.registration.products[0].tests.push({ ...source.registration.products[0].tests[0], methodId: alternateMethod.id, decisionRuleId: null });
    }
