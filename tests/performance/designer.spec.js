...
test('measure production designer parsing, React work, editing and persistence', async ({ page, browser, context }) => {
  ...
  try {
    ...
    for (const fixture of server.fixtures) {
      ...
      await page.locator(`[data-section-id="${selected.sectionId}"] > .template-edit-toolbar`).getByRole('button', { name: 'Edit Mode Off' }).click();
      const saves = [];
      for (let run = -1; run < runs; run += 1) {
        ...
        await column.getByRole('button', { name: (run + 1) % 2 ? 'Move column left' : 'Move column right' }).click();
