...
const watched = ['src/components/organization-settings/OrganizationSettings.jsx', 'src/organization-settings/service.js',
  'src/organization-settings/sample-workflows.js', 'tests/performance/organization-sample-workflows.spec.js'];
...
test('organization workflow controls load and respond with 25 and 1000 published choices', async ({ page }) => {
  ...
  const owner = ownerPool(); const report = { status: 'running', startedAt: new Date().toISOString(),
    ...
    measurement: 'Production Chrome, one warmup/five measurements. Complete navigation, API and six workflow controls through two animation frames; changing IQC selection through two frames. Fixture setup, login, first navigation and screenshots excluded.', cases: [] };
  try {
    ...
    for (const count of [25, 1000]) {
      ...
      for (let iteration = -1; iteration < 5; iteration += 1) {
        ...
        const select = page.getByLabel('IQC Sample', { exact: true }); await expect(select.locator('option')).toHaveCount(count + 1);
        await expect(page.getByLabel('Complaint Sample', { exact: true })).toBeVisible();
    ...
    report.finishedAt = new Date().toISOString(); await writeFile('.local/organization-sample-workflows-browser-performance.json', JSON.stringify(report, null, 2) + '\n');
