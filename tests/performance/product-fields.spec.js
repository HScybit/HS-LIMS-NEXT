...
test('measure Product field loading, React rendering, input and actual saves', async ({ page, context, browser }) => {
  const server = JSON.parse(await readFile('.local/product-fields-performance.json', 'utf8'));
  ...
  const report = { status: 'running', synthetic: true, startedAt: new Date().toISOString(), runs, fixtures: [],
    ...
    conditions: ['Production PROFILE_REACT=1 build, real synthetic tenant data and saves; profiling overhead included.',
      ...
      'Product history arrives through the server-rendered page; the measured JSON parse is the separate current-definition response.',
      'No full source runtime, cold-cache, concurrent-load or memory-leak comparison.'], errors: [] };
  const owner = ownerPool();
  page.on('pageerror', (error) => report.errors.push(error.message));
  try {
    await page.addInitScript(() => {
        const metrics = { responses: [], readyMs: null, longTasks: [] }; window.productFieldBenchmark = metrics;
        const original = window.fetch.bind(window);
        window.fetch = async (...args) => {
          const start = performance.now(); const response = await original(...args);
          if (String(args[0]).startsWith('/api/masters/products')) {
            response.json = async () => {
        ...
        const observer = new PerformanceObserver((entries) => {
          if (!entries.getEntries().some((entry) => entry.name === 'product-fields:react-mount')) return;
    ...
    for (const fixture of server.fixtures) {
      ...
      const result = { name: fixture.name, fieldCount: fixture.fieldCount, productId: fixture.productId, conditions: [], saves: [] };
      ...
      for (const throttled of [false, true]) {
        ...
        for (let run = -1; run < runs; run++) {
          ...
          await expect.poll(() => page.evaluate(() => window.productFieldBenchmark?.readyMs), { timeout: 30_000 }).toBeGreaterThan(0);
          await expect.poll(() => page.evaluate(() => performance.getEntriesByName('product-fields:react-mount').length), { message: 'Build with PROFILE_REACT=1.' }).toBeGreaterThan(0);
          await expect.poll(() => page.evaluate(() => window.productFieldBenchmark.responses.filter((response) => response.path.startsWith('/api/masters/products/custom-field-users')).length)).toBe(1);
          const inputText = `Synthetic ${throttled ? 'throttled' : 'local'} browser edit ${run}`;
          const sample = await page.evaluate(async ({ firstId, value }) => {
            const metrics = window.productFieldBenchmark;
            const before = performance.getEntriesByName('product-fields:react-update').length;
            ...
            const updates = performance.getEntriesByName('product-fields:react-update').slice(before);
            const definition = metrics.responses.find((response) => response.path === '/api/masters/products/custom-fields');
            return { readyMs: metrics.readyMs, definitionApiMs: definition.apiMs, definitionParseMs: definition.parseMs,
              definitionCharacters: definition.characters, reactMountMs: performance.getEntriesByName('product-fields:react-mount')[0].duration,
          ...
          }, { firstId: `product-custom-field-${fixture.fieldIds[0]}`, value: inputText });
          expect(sample.inputCommits).toBeGreaterThan(0);
          expect(sample.apiRequests.filter((path) => path.startsWith('/api/masters/products/custom-field-users'))).toHaveLength(1);
        ...
        await writeFile('.local/product-fields-browser-performance.json', JSON.stringify(report, null, 2));
      }
  ...
  finally { report.finishedAt = new Date().toISOString(); await writeFile('.local/product-fields-browser-performance.json', JSON.stringify(report, null, 2)); await owner.end(); }
});
