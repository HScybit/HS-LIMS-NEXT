...
test('rule selection prefers active then newest valid estimates and falls through zero to inactive rules', async () => {
  ...
  await owner.query('UPDATE decision_rules SET estimated_time_in_days=3,revision=revision+1 WHERE organization_id=$1 AND id=$2', [manager.organizationId, fixture.rule.id]);
  // Active-rule uniqueness (product+parameter+method) now rejects two simultaneously active rules for the
  // same triple, so swap which one is active instead of activating a second one alongside the first: with
  // both estimates now valid ("days IS NOT NULL" ties), the query's next tiebreak is "active DESC", so
  // making alternative the sole active row still selects its days=11 estimate exactly as before.
  await owner.query('UPDATE decision_rules SET active=false,revision=revision+1 WHERE organization_id=$1 AND id=$2', [manager.organizationId, fixture.rule.id]);
