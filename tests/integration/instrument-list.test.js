...
async function fixture() {
  ...
  return { actor, reader, modules, laboratoryId };
}
...
test('Instrument list follows worst canonical service status, stable ties, dates, empty schedules and inactive records', async () => {
  ...
  const scheduled = await save(context, { name: 'Scheduled', serviceConfigurations: [
    service('calibration', { lastPerformedOn: day(-20), nextReminderOn: day(-2) }),
  ...
  await save(context, { name: 'Due today', serviceConfigurations: [service('calibration', { lastPerformedOn: day(-3), frequencyDays: 3 })] });
  await save(context, { name: 'Due soon', serviceConfigurations: [service('calibration', { nextReminderOn: day(2) })] });
...
test('Instrument status uses the supplied viewer calendar day at due-date boundaries', async () => {
  const context = await fixture(); await save(context, { serviceConfigurations: [service('calibration', { nextReminderOn: '2026-09-17' })] });
...
test('Instrument choices match source Unicode search, retain inactive selections and scope service references', async () => {
  ...
  assert.deepEqual((await work(context.actor, instrumentServiceTypes, true)).rows.map(row => row.serviceCode),
    ['preventive_maintenance', 'breakdown', 'calibration']);
