...
test('report field bindings and loop flags survive runtime snapshots, draft clones and later edits in eight queries', async () => {
  ...
  assert.equal(original.metrics.queryCount, 9);
...
test('qr_code_widget accepts no source field, cannot be made editable, and its Title/Key survive a draft clone', async () => {
  const account = await createAccount(owner, { permissions: ['templates.manage', 'templates.read'] });
  const session = await signIn({ identifier: account.username, password: account.password });
  const work = (callback, options) => withSession(session.token, callback, { csrfToken: session.csrfToken, ...options });
  const template = await work(createReportTemplate);
  const field = template.records.fields.find((field) => field.widget === 'tr_data_widget');
  await work((client, identity) => editTemplate(client, identity, template.versionId, 1, { type: 'delete', kind: 'field', id: field.id }));
  await assert.rejects(work((client, identity) => editTemplate(client, identity, template.versionId, 2,
    { type: 'configureField', columnId: field.columnId, widget: 'qr_code_widget', alias: field.alias, sourceField: 'parameterName' })), { code: 'invalid_context_field' });
  await work((client, identity) => editTemplate(client, identity, template.versionId, 2,
    { type: 'configureField', columnId: field.columnId, widget: 'qr_code_widget', alias: field.alias, label: 'Sample QR' }));
  await assert.rejects(work((client, identity) => editTemplate(client, identity, template.versionId, 3,
    { type: 'configureField', columnId: field.columnId, widget: 'qr_code_widget', alias: field.alias, label: 'Sample QR', editable: true })), { code: 'readonly_context_widget' });
  await work((client, identity) => freezeTemplate(client, identity, template.versionId, 3));
  const versionId = await work((client, identity) => resolveCaptureVersion(client, identity, template.templateId, { kind: 'report' }));
  const draft = await work((client, identity) => createDraft(client, identity, versionId));
  const cloned = await work((client, identity) => loadDefinition(client, identity.organization_id, draft.versionId), { readOnly: true });
  const clonedField = cloned.records.fields.find((item) => item.columnId === field.columnId);
  assert.equal(clonedField.widget, 'qr_code_widget'); assert.equal(clonedField.label, 'Sample QR'); assert.equal(clonedField.sourceField, null);
});
