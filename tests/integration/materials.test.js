...
test('material permissions, immutable history and actual actor guards apply to service and direct SQL access', async () => {
  const { actor, material, input } = await setup({ initialQuantity: '2' });
  const receipt = await post(actor, transaction(material.id)); const reader = await account({ organizationId: actor.organizationId, permissions: ['masters.read'] }, false);
  await saveModuleAccessSettings(actor, emptyModuleAccess().map(module => module.moduleKey === 'inventory' ? { ...module, enabled: true, userIds: [actor.userId, reader.userId] } : module));
...
test('material lists and lookups reject malformed bounds and unsupported filters before querying', async () => {
  ...
  await work(actor, async (client, identity) => {
    const noQueries = { query(sql) {
      if (sql.includes('masters_can_read_party')) return { rows: [{ allowed: true }] };
      assert.fail('Invalid input must fail before any listing SQL.');
    } };
