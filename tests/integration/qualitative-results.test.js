...
test('mixed child inputs retain text, zero, clears and actual history across clones, stale saves and reload', async () => {
  ...
  assert.equal(sheet.metrics.definition.queryCount, 8); assert.equal(sheet.metrics.capture.queryCount, 3);
...
test('qualitative child results complete through the real workflow and remain typed in frozen report and PDF output', async () => {
  ...
  assert.equal(report.metrics.definition.queryCount, 8); assert.equal(report.metrics.capture.queryCount, 0, 'Scalar submissions need no full capture reload.');
