...
test('product SQL boundaries reject incomplete history, late children, unsaved links, hidden-link writes and forged history', async () => {
  ...
  await assert.rejects(work((client) => client.query('DELETE FROM product_sample_categories WHERE organization_id=$1 AND product_id=$2', [account.organizationId, command.id])), { code: '42501' });
