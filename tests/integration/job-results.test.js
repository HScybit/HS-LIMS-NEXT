...
test('foreign captures, forged receipts, missing provenance and invalid numeric inputs fail atomically', async () => {
  const flow = await prepareSubjectJob(owner, creator, analyst, { resultWidget: true });
  ...
  for (const value of ['12abc', 'NA']) await assert.rejects(work(analyst, (client, identity) => saveCapture(client, identity, sheet.capture.instance.id, sheet.capture.revision, [entryInput(field, row, value)])), { code: 'invalid_result_value' });
  await assert.rejects(work(analyst, (client, identity) => saveCapture(client, identity, sheet.capture.instance.id, sheet.capture.revision, [entryInput(field, row, '1.234')])), { code: 'result_decimal_scale' });
