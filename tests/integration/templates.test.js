...
test('relational definition loads in eight queries with explicit repeat dependencies and no JSON columns', async () => {
  ...
  assert.equal(loaded.metrics.queryCount, 8);
  assert.equal(Object.keys(loaded.model.fieldsById).length, 4);
  assert.equal(loaded.model.calculationOrder.length, 2);
  assert.equal(loaded.records.expressions.find((expression) => expression.fieldId === template.records.fields.at(-1).id).nodes.some((node) => node.scope === 'descendants'), true);
...
test('cloning a repeated row creates distinct identities and rewires internal formula references', async () => {
  ...
  assert.equal(cloned.model.expressions[`${calculated.id}:calculate`].nodes.find((node) => node.kind === 'field').fieldId, raw.id);
...
test('a fresh 1000-field definition and its capture use eight plus three actual SQL reads', async () => {
