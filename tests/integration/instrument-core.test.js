...
async function fixture() {
  ...
  return { actor, laboratoryId, modules };
}
...
test('Instrument snapshots preserve service IDs, order, roles and labels across unrelated edits, and reject identity reuse under another type', async () => {
  ...
  const configurations = ['preventive_maintenance', 'calibration'].map(serviceCode => ({ id: randomUUID(), serviceCode, templateId: references.template.templateId,
  ...
  assert.equal(saved.serviceConfigurations[0].serviceLabel, 'Preventive Maintenance'); assert.equal(saved.serviceConfigurations[1].serviceLabel, 'Calibration');
  ...
  const reused = configurations.map((service, index) => index === 1 ? { ...service, serviceCode: 'breakdown' } : service);
  await assert.rejects(save(actor, { id: saved.id, revision: 2, requestId: randomUUID(), serviceConfigurations: reused }), { code: 'invalid_instrument_service_identity' });
...
test('Instrument service configurations reject unsupported service codes', async () => {
  const context = await fixture();
  await assert.rejects(save(context.actor, input(context, { serviceConfigurations: [{ id: randomUUID(), serviceCode: 'inspection', reminderBeforeDays: 0 }] })),
    { code: 'invalid_instrument_service_reference' });
});
...
test('configured Instruments prevent Workflow retirement and Template kind changes', async () => {
  ...
  const record = await save(context.actor, input(context, { serviceConfigurations: [{ id: randomUUID(), serviceCode: 'calibration',
...
test('Instrument saves wait for a Template writer and recheck its committed type change', { timeout: 15000 }, async () => {
  ...
  try {
    ...
    pending = work(context.actor, async (client, identity) => {
      ...
      return saveInstrumentCore(client, identity, input(context, { serviceConfigurations: [{ id: randomUUID(), serviceCode: 'calibration',
        templateId: references.template.templateId, workflowId: references.workflow.workflowId, reminderRoleIds: [context.actor.roleId] }] }));
...
test('a Template kind change waits for the first Instrument capture and then rejects its new reference', { timeout: 15000 }, async () => {
  ...
  const writer = work(context.actor, async (client, identity) => {
    const record = await saveInstrumentCore(client, identity, input(context, { serviceConfigurations: [{ id: randomUUID(), serviceCode: 'calibration',
      templateId: references.template.templateId, workflowId: references.workflow.workflowId, reminderRoleIds: [context.actor.roleId] }] }));
