...
test('parameter capture binds each real child, loads in thirteen statements, and keeps zero and sibling formulas separate', async () => {
  ...
  assert.equal(statementCount, 13); assert.equal(sheet.metrics.definition.queryCount, 8); assert.equal(sheet.metrics.capture.queryCount, 3); assert.equal(sheet.metrics.metadataQueryCount, 2);
