...
test('datasheet reads use one metadata plus nine definition and three capture queries, with tenant/history/approval access enforced', async () => {
  ...
  await work(analyst, async (client, identity) => {
    ...
    try {
      ...
      assert.equal(statements.length, 13); assert.ok(statements.every((statement) => /^(select|with)\b/i.test(statement) && !/\b(insert|update|delete)\b/i.test(statement)));
      assert.equal(result.canExecute, true); assert.equal(result.metrics.definition.queryCount, 9); assert.equal(result.metrics.capture.queryCount, 3);
