...
test('material category retries return the original version and reject a changed payload or actor', async () => {
  ...
  const other = await account({ organizationId: actor.organizationId }, false);
  await saveModuleAccessSettings(actor, emptyModuleAccess().map(module => module.moduleKey === 'inventory' ? { ...module, enabled: true, userIds: [actor.userId, other.userId] } : module));
...
test('material category read and write permissions isolate current and historical data', async () => {
  ...
  const reader = await account({ organizationId: manager.organizationId, permissions: ['masters.read'] }, false);
  await saveModuleAccessSettings(manager, emptyModuleAccess().map(module => module.moduleKey === 'inventory' ? { ...module, enabled: true, userIds: [manager.userId, reader.userId] } : module));
...
test('material category guards reject forged metadata, revision gaps, retired changes and historical rewrites', async () => {
  const actor = await account(); const other = await account({ organizationId: actor.organizationId }, false); const category = await save(actor, command());
