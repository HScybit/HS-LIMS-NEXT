...
const moduleDenied = { code: '42501', constraint: 'organization_module_access_required' };
...
async function fixture() {
  ...
  return { manager, actor, customerId, parentId, addressId, contactId, commands };
}
...
test('all nine Customer table writes reject absent, disabled and empty module grants without changing rows', async () => {
  const { manager, actor, commands } = await fixture(); const original = await storedRows(actor);
  for (const state of ['absent', 'disabled', 'empty']) {
    ...
    for (const command of commands) await assert.rejects(execute(actor, command), moduleDenied, `${state}: ${command.name}`);
    assert.deepEqual(await storedRows(actor), original);
  }
});

test('configured direct grants allow committed Customer, address and contact writes with existing identity and revision guards', async () => {
  const { manager, actor, customerId, parentId, commands } = await fixture(); await configure(manager, actor);
  await assert.rejects(work(actor, client => client.query("UPDATE customers SET name='Bad revision' WHERE organization_id=$1 AND id=$2", [actor.organizationId, customerId])), { code: '23514' });
  await assert.rejects(work(actor, client => client.query('UPDATE customers SET id=$3,revision=revision+1 WHERE organization_id=$1 AND id=$2', [actor.organizationId, customerId, randomUUID()])), { code: '23514' });
  for (const command of commands) assert.equal((await execute(actor, command)).rowCount, 1, command.name);
  const rows = await storedRows(actor);
  assert.equal(rows.customers.length, 2); assert(rows.customers.some(row => row.id === commands[0].values[1]));
  assert(!rows.customers.some(row => row.id === customerId)); assert(rows.customers.some(row => row.id === parentId));
  assert.equal(rows.customer_addresses[0].id, commands[3].values[1]); assert.equal(rows.customer_contacts[0].id, commands[6].values[1]);
});
...
test('Customer Default Role grants use the recorded profile and do not grant master-write permission or foreign access', async () => {
  const { manager, actor, commands } = await fixture();
  await configure(manager, actor, { userIds: [], roleIds: [actor.roleId] });
  await assert.rejects(execute(actor, commands[0]), moduleDenied);
  await recordProfile(manager, actor, { roleIds: [actor.roleId, manager.roleId] });
  assert.equal((await execute(actor, commands[0])).rowCount, 1);
  await configure(manager, actor, { userIds: [], roleIds: [manager.roleId] });
  await assert.rejects(execute(actor, { ...commands[0], values: [actor.organizationId, randomUUID()] }), moduleDenied);
  ...
  assert.equal((await execute(foreign, commands[1])).rowCount, 0);
...
test('raw Customer writes recheck committed settings, role and Default Role revocations after native writer locks', { timeout: 30000 }, async () => {
  for (const kind of ['settings', 'role', 'profile']) {
    ...
    try {
      ...
      pending = work(actor, async client => { pid = (await client.query('SELECT pg_backend_pid() AS pid')).rows[0].pid; return client.query(commands[0].sql, commands[0].values); });
      void pending.catch(error => { failure = error; }); await waitForLock(() => pid, () => failure ?? editorFailure); release(); await editing;
      await assert.rejects(pending, { code: '42501' });
      assert.equal((await owner.query('SELECT id FROM customers WHERE organization_id=$1 AND id=$2', commands[0].values)).rowCount, 0);
    } finally { release(); await editing.catch(() => {}); if (pending) await pending.catch(() => {}); }
  }
});

test('raw Customer writes recheck session, membership, user, organization, credential and expiry changes after waiting', { timeout: 30000 }, async () => {
  for (const kind of ['session', 'membership', 'user', 'organization', 'credential', 'expiry']) {
    const { manager, actor, commands } = await fixture(); await configure(manager, actor); const original = await storedRows(actor);
    ...
    try {
      ...
      pending = work(actor, async client => { pid = (await client.query('SELECT pg_backend_pid() AS pid')).rows[0].pid; return client.query(commands[1].sql, commands[1].values); });
      ...
      await blocker.query('COMMIT'); await assert.rejects(pending, { code: '42501' }); assert.deepEqual(await storedRows(actor), original);
  ...
  for (const isolation of ['READ COMMITTED', 'READ UNCOMMITTED', 'REPEATABLE READ', 'SERIALIZABLE']) {
    ...
    try {
      ...
      if (['READ COMMITTED', 'READ UNCOMMITTED'].includes(isolation)) {
        await saveModuleAccessSettings(manager, emptyModuleAccess());
        await assert.rejects(client.query(commands[0].sql, commands[0].values), moduleDenied);
      } else {
        for (const command of commands) {
          await client.query('SAVEPOINT isolation_probe'); await assert.rejects(client.query(command.sql, command.values), isolationDenied);
...
test('Customer write authorization stays bound to the authenticated organization throughout multirow statements', async () => {
  ...
  for (const command of customerScopeCommands(own, sameTenant)) {
    await assert.rejects(work(actor, async client => {
      const result = await client.query(command.sql, command.values);
      assert.equal(result.rowCount, 2, command.name);
      assert(result.rows.every(row => row.organization_id === actor.organizationId));
      throw rollback;
  ...
  for (const accessPath of ['automatic', 'sequential']) {
    for (const command of customerScopeCommands(own, otherTenant)) {
      const attempt = () => work(actor, async client => {
        ...
        const result = await client.query(command.sql, command.values);
        assert(!command.name.endsWith(' insert'), 'A mixed-tenant INSERT must fail atomically');
        assert(result.rowCount <= 1, command.name);
        assert(result.rows.every(row => row.organization_id === actor.organizationId));
        assert(result.rows.every(row => row.attempted_scope === foreign.organizationId));
        throw rollback;
      });
      await assert.rejects(attempt, error => error === rollback || error.code === '42501', `${accessPath}: ${command.name}`);
