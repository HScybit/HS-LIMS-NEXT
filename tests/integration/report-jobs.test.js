...
before(async () => {
  ...
  assert.deepEqual(functions.rows.map((row) => row.proname), ['laboratory_parameter_context_scope', 'laboratory_parameter_read_scope', 'laboratory_product_context_scope', 'report_final_line_contexts', 'report_pdf_begin_read', 'report_pdf_claim', 'report_pdf_complete', 'report_pdf_context_org', 'report_pdf_fail']);
});
...
test('worker reads require a live lease and cannot cross tenants or mutate analytical records', async () => {
  ...
  await workerWork(async (client) => {
    ...
    assert.equal(report.report.id, flow.reportId); assert.equal(report.metrics.definition.queryCount, 9); assert.equal(report.metrics.capture.queryCount, 3);
