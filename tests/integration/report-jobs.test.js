...
before(async () => {
  ...
  assert.deepEqual(functions.rows.map((row) => row.proname), ['laboratory_parameter_context_scope', 'laboratory_parameter_read_scope', 'laboratory_product_context_scope', 'report_pdf_begin_read', 'report_pdf_claim', 'report_pdf_complete', 'report_pdf_context_org', 'report_pdf_fail']);
});
