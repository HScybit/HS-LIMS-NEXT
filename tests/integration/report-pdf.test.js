...
test('a column marked "show in NABL only" carries that class into the rendered document, and the bundled stylesheet actually hides it in the opposite mode', async () => {
  // The rule lives in custom.scss (one of the sources this same before() block bundled into
  // `stylesheet`) specifically so it reaches real PDF generation, not only the live web preview
  // this exact rule already existed for (ReportFrame.jsx's own inline copy, and a second
  // .finalised-report-preview-scoped copy in finalised-report-page.scss) — verify it's really there.
  assert.match(stylesheet, /\.nabl_mode \.show_in_non_nabl,\.non_nabl_mode \.show_in_nabl\{display:none !important\}/);
  const flow = await prepareReportFlow(owner, account, {});
  const work = (action, options) => withSession(account.token, action, { csrfToken: account.csrfToken, ...options });
  const targetColumn = flow.template.records.columns[0];
  await work((client, identity) => editTemplate(client, identity, flow.template.versionId, flow.template.revision, { type: 'configureColumn', id: targetColumn.id, span: targetColumn.span, cssClass: targetColumn.cssClass, showInNabl: true }));
  // Accreditation must be set before generation — a generated report's own rows are immutable
  // (report_reissue/report_group are enforced at INSERT, not editable after the fact), so is_nabl
  // has to come from the test actually being accredited at the moment the report is produced.
  await owner.query('UPDATE sample_tests SET is_accredited=true WHERE organization_id=$1 AND id=$2', [account.organizationId, flow.sampleTestId]);
  const generated = await work((client, identity) => generateReports(client, identity, flow.sample.id, flow.input));
  const data = await work((client, identity) => loadReport(client, identity, generated.items[0].id), { readOnly: true });
  assert.equal(data.report.isNabl, true);
  const html = renderer.renderReportDocument(data, stylesheet);
  assert.match(html, /nabl_mode/);
  assert.match(html, /show_in_nabl/);
});
...
test('a completed job renders each actual summary result and frozen specification even when a child uses an alternate method', async () => {
  ...
  const data = await work((client, identity) => loadReport(client, identity, generated.items[0].id), { readOnly: true });
  assert.equal(data.results.length, 2); assert.equal(data.metrics.definition.queryCount, 9); assert.equal(data.metrics.capture.queryCount, 0);
...
test('qr_code_widget renders a real, well-formed QR-code SVG for the actual sample number in the frozen renderer bundle', async () => {
  // TemplateCanvas.jsx (which draws this widget) is bundled into `renderer` by the same
  // scripts/build-report-renderer.js step this file's before() runs, and it is the same
  // component ReportContent.jsx uses for the live browser preview — so exercising it here,
  // through the real frozen bundle, covers both surfaces.
  const flow = await prepareReportFlow(owner, account, {});
  const work = (action, options) => withSession(account.token, action, { csrfToken: account.csrfToken, ...options });
  const detailsSection = flow.template.records.sections[1];
  const detailsRow = flow.template.records.rows.find((row) => row.sectionId === detailsSection.id);
  let draft = await work((client, identity) => editTemplate(client, identity, flow.template.versionId, flow.template.revision, { type: 'addColumn', rowId: detailsRow.id }));
  const columnId = draft.model.rowsById[detailsRow.id].columnIds.at(-1);
  await work((client, identity) => editTemplate(client, identity, flow.template.versionId, draft.model.version.revision, { type: 'configureField', columnId, widget: 'qr_code_widget', alias: 'sample_qr' }));
  const generated = await work((client, identity) => generateReports(client, identity, flow.sample.id, flow.input));
  const data = await work((client, identity) => loadReport(client, identity, generated.items[0].id), { readOnly: true });
  const html = renderer.renderReportDocument(data, stylesheet);
  assert.match(html, /<svg xmlns="http:\/\/www\.w3\.org\/2000\/svg" viewBox="0 0 (\d+) \1" shape-rendering="crispEdges">/);
  assert.match(html, /<path fill="#ffffff"/); assert.match(html, /<path stroke="#000000"/);
  assert.match(html, new RegExp(`<figcaption[^>]*>${data.sample.sampleNumber}</figcaption>`));
  assert.equal(html.includes('No value to encode'), false);
});
