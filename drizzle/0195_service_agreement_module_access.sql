-- Service Agreements extends new captures to four configured modules.
-- Historical two- and three-module captures keep their real recorded shape.
ALTER TABLE organization_module_access_versions ALTER COLUMN module_count SET DEFAULT 4;
ALTER TABLE organization_module_access_versions DROP CONSTRAINT module_access_count;
ALTER TABLE organization_module_access_versions ADD CONSTRAINT module_access_count CHECK(module_count IN (2,3,4));
...
ALTER TABLE organization_module_access_modules ADD CONSTRAINT module_access_module_fields
  CHECK(module_key IN ('customer','vendor','instrument','service_agreements') AND role_count BETWEEN 0 AND 500 AND user_count BETWEEN 0 AND 500);
...
BEGIN
  IF (SELECT array_agg(module_key ORDER BY module_key) FROM public.organization_module_access_modules
      ...
      (CASE NEW.module_count WHEN 2 THEN ARRAY['customer','vendor']::text[] WHEN 3 THEN ARRAY['customer','instrument','vendor']::text[] ELSE ARRAY['customer','instrument','service_agreements','vendor']::text[] END) THEN
...
BEGIN
  ...
  FOREACH omitted_module IN ARRAY ARRAY['instrument','service_agreements'] LOOP
  ...
  END IF;
  INSERT INTO public.organization_module_access_versions(organization_id,revision,module_count,saved_by) VALUES(org,p_revision,4,actor);
