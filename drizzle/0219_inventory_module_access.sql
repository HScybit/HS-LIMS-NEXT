-- Adds "Inventory" (materials/material categories/material transactions) as a
-- fifth organization-configurable module, matching PERN's moduleCatalog and
-- App.jsx nav gate (enabledModules.includes("inventory") && masters.read). Of
-- PERN's other catalog entries (template-management, sample-management,
-- workflow-management, data-masters, data-transfer) none actually gate
-- anything in PERN itself — every organization is auto-enabled for them at
-- creation and no route/nav check ever reads them — so only the one module
-- PERN genuinely enforces beyond this app's existing four is added here.
ALTER TABLE organization_module_access_versions DROP CONSTRAINT module_access_count;
ALTER TABLE organization_module_access_versions ADD CONSTRAINT module_access_count CHECK (module_count = ANY (ARRAY[2,3,4,5]));
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.organization_save_module_access(p_revision integer, p_modules text[], p_enabled boolean[], p_role_modules text[], p_role_ids uuid[], p_user_modules text[], p_user_ids uuid[])
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE actor uuid:=public.organization_lock_settings_writer(); org uuid:=public.organization_module_scope();
  head public.organization_laboratory_settings; previous_module public.organization_module_access_modules; omitted_module text;
BEGIN
  ...
  IF (p_modules IS DISTINCT FROM ARRAY['customer','vendor']::text[] AND p_modules IS DISTINCT FROM ARRAY['customer','vendor','instrument']::text[]
    AND p_modules IS DISTINCT FROM ARRAY['customer','vendor','instrument','service_agreements']::text[]
    AND p_modules IS DISTINCT FROM ARRAY['customer','vendor','instrument','service_agreements','inventory']::text[])
    ...
    OR cardinality(p_role_ids) IS NULL OR cardinality(p_role_ids)>2000 OR cardinality(p_role_modules) IS DISTINCT FROM cardinality(p_role_ids)
    OR cardinality(p_user_ids) IS NULL OR cardinality(p_user_ids)>2000 OR cardinality(p_user_modules) IS DISTINCT FROM cardinality(p_user_ids)
  ...
  END IF;
  -- Preserve modules absent from an older caller's complete supported prefix.
  -- The settings writer holds the organization and current settings head locks.
  FOREACH omitted_module IN ARRAY ARRAY['instrument','service_agreements','inventory'] LOOP
    IF omitted_module=ANY(p_modules) THEN CONTINUE; END IF;
    SELECT * INTO previous_module FROM public.organization_module_access_modules module
      WHERE module.organization_id=org AND module.module_key=omitted_module
    ...
    p_modules:=array_append(p_modules,omitted_module);
    p_enabled:=array_append(p_enabled,coalesce(previous_module.enabled,false));
    IF previous_module.module_key IS NOT NULL THEN
      SELECT p_role_modules||coalesce(array_agg(module_key ORDER BY position),ARRAY[]::text[]),
        ...
        WHERE organization_id=org AND revision=previous_module.revision AND module_key=omitted_module;
  ...
  END IF;
  INSERT INTO public.organization_module_access_versions(organization_id,revision,module_count,saved_by) VALUES(org,p_revision,5,actor);
...
END $function$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.masters_can_read_party(requested_module text)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  SELECT requested_module IN ('customer','vendor','inventory')
...
$function$;
--> statement-breakpoint
CREATE FUNCTION masters_require_inventory_write() RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
BEGIN
  PERFORM public.masters_lock_field_writer();
  PERFORM public.organization_require_module_access('inventory');
  RETURN true;
END $$;
REVOKE ALL ON FUNCTION masters_require_inventory_write() FROM PUBLIC,sampleify_report_worker;
GRANT EXECUTE ON FUNCTION masters_require_inventory_write() TO sampleify_app;
--> statement-breakpoint
CREATE POLICY material_module_read ON materials AS RESTRICTIVE FOR SELECT TO sampleify_app USING (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_can_read_party('inventory'));
CREATE POLICY material_module_insert ON materials AS RESTRICTIVE FOR INSERT TO sampleify_app WITH CHECK (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write());
CREATE POLICY material_module_update ON materials AS RESTRICTIVE FOR UPDATE TO sampleify_app
  USING (NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write())
  WITH CHECK (NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write());
--> statement-breakpoint
CREATE POLICY material_category_module_read ON material_categories AS RESTRICTIVE FOR SELECT TO sampleify_app USING (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_can_read_party('inventory'));
CREATE POLICY material_category_module_insert ON material_categories AS RESTRICTIVE FOR INSERT TO sampleify_app WITH CHECK (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write());
CREATE POLICY material_category_module_update ON material_categories AS RESTRICTIVE FOR UPDATE TO sampleify_app
  USING (NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write())
  WITH CHECK (NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write());
--> statement-breakpoint
CREATE POLICY material_transaction_module_read ON material_transactions AS RESTRICTIVE FOR SELECT TO sampleify_app USING (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_can_read_party('inventory'));
CREATE POLICY material_transaction_module_insert ON material_transactions AS RESTRICTIVE FOR INSERT TO sampleify_app WITH CHECK (
  NOT (organization_id IS DISTINCT FROM organization_module_scope()) AND masters_require_inventory_write());
--> statement-breakpoint
ALTER TABLE organization_module_access_modules DROP CONSTRAINT module_access_module_fields;
ALTER TABLE organization_module_access_modules ADD CONSTRAINT module_access_module_fields CHECK (
  module_key = ANY (ARRAY['customer','vendor','instrument','service_agreements','inventory'])
  AND role_count BETWEEN 0 AND 500 AND user_count BETWEEN 0 AND 500);
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.organization_check_module_access_complete() RETURNS trigger
...
BEGIN
  IF (SELECT array_agg(module_key ORDER BY module_key) FROM public.organization_module_access_modules
      ...
      (CASE NEW.module_count
        WHEN 2 THEN ARRAY['customer','vendor']::text[]
        WHEN 3 THEN ARRAY['customer','instrument','vendor']::text[]
        WHEN 4 THEN ARRAY['customer','instrument','service_agreements','vendor']::text[]
        ELSE ARRAY['customer','instrument','inventory','service_agreements','vendor']::text[] END) THEN
