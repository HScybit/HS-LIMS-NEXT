-- Create-only Customer uploads retain typed provenance and every existing cell FK.
...
ALTER TABLE master_bulk_batches ADD CONSTRAINT master_bulk_batch_fields CHECK (
  resource IN ('products','test-parameters','methods','users','customers')
...
ALTER TABLE master_bulk_attempts ADD COLUMN customer_id uuid;
ALTER TABLE master_bulk_attempts ADD CONSTRAINT master_bulk_attempt_customer_fk
  FOREIGN KEY (organization_id,customer_id,result_revision) REFERENCES customer_versions(organization_id,customer_id,revision);
...
BEGIN
  ...
  -- Hold the same authorization locks as native Customer saves for each mutation.
  IF upload.resource='customers' AND (TG_TABLE_NAME IN ('master_bulk_batches','master_bulk_reviews','master_bulk_attempts') OR TG_OP='UPDATE') THEN
  ...
  IF TG_TABLE_NAME='master_bulk_reviews' THEN
    IF upload.resource='customers' AND NEW.valid AND (NEW.operation IS DISTINCT FROM 'create' OR NEW.expected_revision IS DISTINCT FROM 0) THEN
      RAISE EXCEPTION 'Customer uploads create records only' USING ERRCODE='23514';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF NEW.committed THEN
    IF NOT reviewed.valid OR NEW.result_revision<>reviewed.expected_revision+1
      OR coalesce(NEW.product_id,NEW.parameter_id,NEW.method_id,NEW.user_id,NEW.customer_id) IS DISTINCT FROM reviewed.candidate_id THEN
