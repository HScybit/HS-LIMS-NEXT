...
import { emptyModuleAccess, grantSyntheticInventoryAccess, moduleAccessValues, saveModuleAccessSettings } from '../tests/helpers/module-access.js';
...
try {
  ...
  await saveModuleAccessSettings(instrumentActor, instrumentModules);
  ...
  await withSession(session.token, async (client, identity) => {
    ...
    await saveLaboratorySettings(client, identity, { revision: 0, autoCreateJobs: false, resultSummaryTemplateId: null, jobWorkflowId: null, selfAllocationEnabled: true,
      dateFormat: 'Do MMMM YYYY', datetimeFormat: 'MMMM Do YYYY | hh:mm A', moduleAccess });
    ...
    assert.equal((await loadModuleAccess(client, identity)).moduleCount, emptyModuleAccess().length);
  ...
  // Material categories and materials are gated by the Inventory module, which
  // the customer-module block above leaves disabled.
  await grantSyntheticInventoryAccess(owner, account);
  await withSession(session.token, async (client, identity) => {
