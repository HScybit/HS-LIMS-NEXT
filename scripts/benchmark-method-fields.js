...
import { createMethodFieldFixture } from '../tests/helpers/method-field-fixtures.js';
import { signIn, withSession } from '../src/auth/service.js';
import { closePool } from '../src/db/pool.js';
import { methodCustomFields } from '../src/masters/custom-fields.js';
import { loadMethod, saveMethod, listMethods } from '../src/masters/methods.js';
import { generateMethodCustomFields } from '../src/masters/method-custom-field-generation.js';

const fixtures = [
  { name: 'small', fieldCount: 10, methodCount: 10, readP95Ms: 150, listP95Ms: 200, saveP95Ms: 500, generationP95Ms: 400, payloadBytes: 131072 },
  { name: 'large', fieldCount: 100, methodCount: 10, readP95Ms: 400, listP95Ms: 600, saveP95Ms: 1500, generationP95Ms: 700, payloadBytes: 1572864 },
  { name: 'complex', fieldCount: 500, methodCount: 100, readP95Ms: 1500, listP95Ms: 3500, saveP95Ms: 6000, generationP95Ms: 1500, payloadBytes: 16777216 },
];
...
await writeFile('.local/method-fields-performance-budgets.json', JSON.stringify(budgets, null, 2));
const sourceFiles = ['src/masters/methods.js', 'src/masters/method-custom-fields.js', 'src/masters/method-custom-field-listing.js',
  'src/masters/method-custom-field-generation.js', 'src/custom-fields/product-generation-worker.js', 'tests/helpers/method-field-fixtures.js'];
...
try {
  ...
  for (const fixture of fixtures) {
    ...
    const data = await createMethodFieldFixture(work, { ...fixture, userId: account.userId });
    let current = data.methods[0];
    const result = { ...fixture, organizationId: account.organizationId, methodId: current.id, fieldIds: data.fields.map((field) => field.id),
    ...
    console.log(`${fixture.name}: ${fixture.fieldCount} definitions and ${fixture.methodCount} captured Methods created.`);
    const generationInput = { method: data.method, customFieldTimeZone: 'UTC',
      customFields: data.values.map((value, index) => ({ ...value, value: index >= fixture.fieldCount - 3 ? '' : value.value })) };
    const operations = {
      definitions: (client, identity) => methodCustomFields(client, identity),
      load: (client, identity) => loadMethod(client, identity, current.id),
      list: (client, identity) => listMethods(client, identity, { pageSize: fixture.methodCount, sort: { key: `pf:${data.fields[0].id}`, dir: 'asc' } }),
      generation: (client, identity) => generateMethodCustomFields(client, identity, generationInput),
      save: async (client, identity) => {
        current = await saveMethod(client, identity, { ...data.method, uuid: current.uuid, id: current.id, revision: current.revision,
    ...
    for (const [name, action] of Object.entries(operations)) {
      ...
      for (let run = -budgets.warmups; run < budgets.samples; run++) {
        ...
        await work(async (client, identity) => {
          ...
          if (name === 'list') assert.equal(output.rows.length, fixture.methodCount);
          if (name === 'generation') assert.deepEqual(output.values.map((value) => value.value), ['M/002', 'M/002/copy', 'M/002/copy/copy']);
      ...
      await writeFile('.local/method-fields-performance.json', JSON.stringify(report, null, 2));
    }
  }
